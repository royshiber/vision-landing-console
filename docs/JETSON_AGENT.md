...
`scripts/jetson-companion/companion_agent.py` (גרסה `2.3.10`) מגיש סטטוס ניווט אופטי לצפייה בלבד, צינור מצלמות כנה, סטטוס מודם סלולר מקובץ המארח, שליטה בקישורי רשת, וסטטוס ראייה מסומנת כנה:
...
# expected: GET /api/health agentVersion 2.3.10
