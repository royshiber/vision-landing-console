...
`scripts/jetson-companion/companion_agent.py` (גרסה `2.3.8`) מגיש סטטוס ניווט אופטי לצפייה בלבד, צינור מצלמות כנה, סטטוס מודם סלולר מקובץ המארח, וסטטוס ראייה מסומנת כנה:
...
# expected: GET /api/health agentVersion 2.3.8
