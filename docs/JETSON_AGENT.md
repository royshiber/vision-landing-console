...
`scripts/jetson-companion/companion_agent.py` (גרסה `2.6.3`) מגיש סטטוס ניווט אופטי לצפייה בלבד, צינור מצלמות כנה, זווית ראייה שמורה לכל מצלמה, סטטוס מודם סלולר מקובץ המארח, שליטה בקישורי רשת, גיבויי גרסה, וסטטוס ראייה מסומנת כנה:
...
# expected: GET /api/health agentVersion 2.6.3
...
לחיצה על **התחבר** בשורת רדיו טלמטריה פותחת TCP לממסר של הסוכן (`host:5770` מכתובת הבסיס). בלי הקלדת מארח. ניתוק הרדיו סוגר את הממסר ומנקה זיכרון ישן. ניתוק רשת בית משאיר את הממסר פתוח ואינו מוחק טוקן. אם הדופק בבקר חי והממסר נכשל — הקונסול מציג דופק חי בלי מדדים מזויפים.
