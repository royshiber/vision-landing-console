...
24. **Telemetry dash MERGE** — **this draft.** `#telemetry` stays reachable. Platform **אבחונים** opens the existing panel. Primary ops tab is quieted. Pulse keeps compressed companion/link/aircraft. No GPS invented. Companion B2 / Jetson install-rollback writes stay gated / deferred (#28).

---

## Audit snapshot — 2026-09-07 (Telemetry dash MERGE this draft)

**Master:** `1.02.255` @ `6b6218f` (PR #68 merge — Flight Engineer→Assist fold). Do not bump.  
**Prior pointer:** 2026-09-07 Flight Engineer→Assist draft snapshot. Superseded. #68 landed FE fold.  
**This draft:** Telemetry dash MERGE into Pulse (compressed) + Platform diagnostics. Stay draft. King merges after independent VERIFY. Do not bump `APP_VERSION`.
...
- Telemetry dash MERGE. Platform shell **אבחונים** opens existing `#telemetry`. Primary ops tab is quieted (`tab-quiet`, hidden). Pulse first-open uses **אבחונים**. Pulse companion/link/aircraft glance unchanged. Panel stays; no cliff-delete. Companion B2 policy editor and Jetson install-rollback writes untouched. No GPS invented. Mission Assist, Attention Policy, Advisor/FE Assist folds stay. `APP_VERSION` stays `1.02.255`.
...
- **Telemetry dash MERGE this draft:** Pulse first-open label is **אבחונים**. Companion/link/aircraft glance stays compressed. `#telemetry` is no longer a loud ops peer.
...
| PR | Topic | This map |
|---|---|---|
| this draft | Telemetry dash MERGE into Pulse + Platform diagnostics | **This draft.** Stay draft. King merges after independent VERIFY. |
...
1. **Telemetry dash MERGE** — this draft. Platform **אבחונים** → existing `#telemetry`. Pulse stays home. Panel stays. No GPS. No Jetson write.
...
Do not spend cycles on smoke #4, leftover disconnected chrome, version bump, 300+ hunt, inventing a Companion token, Jetson apply/restart, flight commands, another Pulse polish pass, another taxonomy polish pass, another evidence-strip polish pass, Companion B2 policy rewrite, C10.7b voice/STT, or C10.4c-b Jetson deploy-wire move. #50 / #52 / #54 / #56 / #58 / #59 / #61 / #62 / #64 / #65 / #66 / #67 / #68 are landed. Do not invent a token. Do not invent GPS. Do not fake a live Jetson connect. Do not add ElevenLabs/STT keys.

### Next pick (this snapshot)

**Telemetry dash MERGE** is this draft. Flight Engineer fold landed via #68 @ `6b6218f`. C10.7b voice/STT stays deferred. Remainder **C10.4c-b** Jetson deploy-wire move stays deferred (#28). Companion B2 / Jetson install-rollback writes stay out of this fold. Jetson live connect stays parked until Roy supplies the Companion token (days; Jetson `:8081` → 401 without it). Human Gates stay **WAITING FOR ROY**: #28 apply/restart, #29 live vehicle / auto-land. Merge autonomy still on for verified safe draft PRs. Measure by capabilities closed, not PR count. `APP_VERSION` stays `1.02.255`.
