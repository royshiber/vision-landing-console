# Cam0 deploy (Jetson, companion 2.6.2)
...
Expect `agentVersion` `2.6.2`. CAM1 is idle until a client opens `/api/v1/cam1/stream.mjpg`. Health still names `/dev/airvix-cam1` while idle.
