...
export function buildUpdaterLaunch({ platform = process.platform, appRoot, logPath, statusPath, rollback = false } = {}) {
  ...
  if (platform === 'win32') {
    ...
    return {
      ...
      args: ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-WindowStyle', 'Hidden', '-File', script, rollback ? '-Rollback' : '-UpdateOnly'],
      cwd: root,
  ...
  return {
    ...
    script,
    command: 'bash',
    args: rollback ? [script, '--rollback'] : [script],
    cwd: root,
    logPath: log,
    statusPath: status,
    env: {
...
export function createAppUpdateService(opts = {}) {
  ...
  async function rollbackPrevious({ confirmUnknown = false, mavConn = null } = {}) {
    const safety = assessUpdateSafety(mavConn, { confirmUnknown });
    if (!safety.allowed) {
      return {
        ok: false,
    ...
    const launch = buildUpdaterLaunch({ platform, appRoot, logPath, statusPath, rollback: true });
    if (!existsSync(launch.script)) {
      return {
        ok: false,
        status: 500,
