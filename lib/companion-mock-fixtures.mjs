...
export function opticalNavStub({ present = false, running = false, cameraOk = false, confidence = null, note } = {}) {
  return {
    present,
    running,
    camera_ok: cameraOk,
    ...
    display_only: true,
    cameras: {
      cam1: { id: "cam1", role: "vio_forward", shared_with: "landing_vision", camera_ok: cameraOk, enabled: true },
      cam2: { id: "cam2", role: "optical_flow_down", shared_with: "landing_vision", camera_ok: cameraOk, enabled: true },
      cam3: {
        id: "cam3",
        role: "gimbal_observe",
    ...
    note: note || "observe-only optical-nav; position/velocity null until real pipeline; not EKF fused",
  };
}
  ...
  return {
    ...
    fc: {
      ...
      sys_status: {
        ...
        system_id: 1,
        component_id: 1,
        fields: { load_pct: 18, mem_pct: 44, temperature_c: 36.2 },
      },
    ...
    optical_nav: opticalNavStub({
      present: true,
      running: true,
      cameraOk: true,
      confidence: 0.75,
      note: "mock optical-nav observe-only; cameras shared with landing/vision; no WGS84 until real pipeline",
    }),
    landing: {
      timestamp: { ...TS },
      source: "aruco",
...
export function disconnectedCompanionStatus() {
  return {
    ...
    optical_nav: opticalNavStub({
      ...
      running: false,
      cameraOk: false,
      note: "companion disconnected; optical-nav absent; position null",
    }),
    landing: {
      timestamp: { ...TS },
      source: "none",
...
export function apiUpFcDownCompanionStatus() {
  return {
    ...
    extras: { fc_connected: false, camera_connected: false, optical_nav: opticalNavStub() },
    optical_nav: opticalNavStub({
      present: false,
      running: false,
      cameraOk: false,
      note: "API up; cameras absent; optical-nav stub; position null",
    }),
  };
}
...
export function degradedCompanionStatus() {
  ...
  s.optical_nav = opticalNavStub({
    present: true,
    running: true,
    cameraOk: false,
    note: "mock degraded; camera_ok false; position null; not EKF fused",
  });
  return s;
}
...
/** Deterministic release catalog for mock mode (clearly MOCK — no real deploy engine). */
export const MOCK_RELEASE_CATALOG = Object.freeze([
  Object.freeze({
    release_id: 'rel-active-001',
    version: '1.0.1-mock',
    status: 'ACTIVE',
    created_at: '2026-08-18T10:00:00Z',
    compatibility: 'jetson-orin',
    sha256: 'a1b2c3d4e5f6789012345678901234567890abcdef1234567890abcdef123456',
    size_bytes: 48_500_000,
  }),
  Object.freeze({
    release_id: 'rel-prev-002',
    version: '1.0.0-mock',
    status: 'PREVIOUS',
  ...
  Object.freeze({
    ...
    compatibility: 'jetson-orin',
    sha256: 'c3d4e5f6789012345678901234567890abcdef1234567890abcdef1234567ab1',
    size_bytes: 49_100_000,
  }),
  Object.freeze({
    release_id: 'rel-available-004',
    version: '1.0.3-mock',
    status: 'AVAILABLE',
    created_at: '2026-08-19T12:00:00Z',
    compatibility: 'jetson-orin',
    sha256: 'd4e5f6789012345678901234567890abcdef1234567890abcdef1234567ab12c',
    size_bytes: 49_800_000,
  }),
  Object.freeze({
    release_id: 'rel-failed-005',
    version: '0.9.9-mock',
    status: 'FAILED',
    created_at: '2026-08-10T06:00:00Z',
    compatibility: 'jetson-orin',
    sha256: 'e5f6789012345678901234567890abcdef1234567890abcdef1234567ab12cd3',
    size_bytes: 45_000_000,
  }),
]);

export function releaseInventoryForScenario(scenario, { deployState = 'IDLE' } = {}) {
  if (scenario === 'disconnected') {
    return null;
  }
  const active = MOCK_RELEASE_CATALOG[0];
  const previous = MOCK_RELEASE_CATALOG[1];
  const available = scenario === 'degraded'
    ? MOCK_RELEASE_CATALOG.filter((r) => r.release_id !== 'rel-available-004')
...
export function backupsForScenario(scenario) {
  if (scenario === 'disconnected') return null;
  return {
    timestamp: { ...TS },
    backups: [
      {
        backup_id: 'bk-mock-001',
        created_at: '2026-08-19T08:00:00Z',
        release_id: 'rel-active-001',
        sha256: 'f6789012345678901234567890abcdef1234567890abcdef1234567ab12cd34',
      },
      {
        backup_id: 'bk-mock-002',
        created_at: '2026-08-18T10:05:00Z',
        release_id: 'rel-prev-002',
        sha256: '789012345678901234567890abcdef1234567890abcdef1234567ab12cd3456',
      },
    ],
  };
}

export function auditForScenario(scenario) {
  if (scenario === 'disconnected') return null;
  return {
    timestamp: { ...TS },
    entries: [
      {
        timestamp: { t_monotonic_ns: TS.t_monotonic_ns + 3_000_000, t_utc_ns: null },
        operation: 'deploy',
        release_id: 'rel-active-001',
        result: 'success',
        failure_reason: null,
        active_release_id: 'rel-active-001',
      },
      {
        timestamp: { t_monotonic_ns: TS.t_monotonic_ns + 2_000_000, t_utc_ns: null },
        operation: 'backup',
        release_id: 'rel-active-001',
        result: 'success',
        failure_reason: null,
      ...
      },
      {
        ...
        result: 'success',
        failure_reason: null,
        active_release_id: 'rel-prev-002',
      },
    ],
  };
}

export function findMockReleaseById(releaseId) {
  return MOCK_RELEASE_CATALOG.find((r) => r.release_id === releaseId) || null;
}
