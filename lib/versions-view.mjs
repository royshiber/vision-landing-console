...
function blockLine(detail) {
  return `לא ניתן לשחזר כרגע: ${detail}`;
}

export const ARMED_HE = blockLine('המטוס חמוש.');
export const ROLLBACK_TIMEOUT_MS = 45000;
export const ROLLBACK_BLOCK_HE = Object.freeze({
  armed: ARMED_HE,
  in_flight: blockLine('המטוס באוויר.'),
  unknown: blockLine('מצב הטיסה לא ידוע.'),
  stale: blockLine('מצב הטיסה לא עדכני.'),
  missing: blockLine('אין מצב טיסה ממחשב המשימה.'),
  busy: blockLine('שחזור כבר מתבצע.'),
...
/** Install time from a deploy manifest, then version.js mtime, then the git commit date. */
export function readTreeStamp(dir) {
  if (!dir) return { at: null, sha: null };
  let sha = null;
  try {
    const parsed = JSON.parse(readFileSync(path.join(dir, 'airvix-deploy.json'), 'utf8'));
    if (parsed && typeof parsed === 'object') {
      if (typeof parsed.git_sha === 'string' && parsed.git_sha.trim()) sha = parsed.git_sha.trim();
      const at = firstStamp([parsed.deployed_at, parsed.installed_at, parsed.installedAt]);
      if (at) return { at, sha };
    }
  } catch {
    /* no manifest */
  ...
  try {
    const mtime = statSync(path.join(dir, 'version.js')).mtime;
    if (mtime instanceof Date && !Number.isNaN(mtime.getTime())) {
      return { at: mtime.toISOString(), sha };
    }
  } catch {
    /* version.js is not on disk */
  }
  try {
    const at = execFileSync('git', ['log', '-1', '--format=%cI', '--', 'version.js'], {
      cwd: dir,
      encoding: 'utf8',
      timeout: 2000,
  ...
  for (const file of candidates) {
    ...
    if (version) {
      return {
        ...
        version,
        versionFile: file,
        installedAt: readTreeStamp(path.dirname(file)).at,
      };
    }
  }
  return { available: false, version: null, versionFile: null, installedAt: null };
}
...
export function buildVersionsView({
  ...
  return {
    ...
    console: {
      ...
      installedAt: state?.console?.installedAt || consoleInstalledAt || null,
      previousInstalledAt: previousConsole?.installedAt || null,
...
export function assertRollbackConfirm(body, expected) {
  ...
  if (!expected?.from) {
    return { ok: false, status: 409, message: blockLine('גרסה לא ידועה.') };
  }
