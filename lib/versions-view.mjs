...
export const UNKNOWN_HE = 'אין מידע';
export const NO_COMPANION_HE = 'אין קישור למחשב המשימה';
export const NO_CONSOLE_ROLLBACK_HE = 'אין עותק של הגרסה הקודמת. החזרה לא זמינה.';
export const ARMED_HE = 'המטוס חמוש. אין החזרה.';

const EMPTY_STATE = {
  console: { version: null, installedAt: null, previousVersion: null },
  knownGood: null,
  autoFlightId: null,
  rollback: { state: 'idle', kind: null, from: null, to: null, backupId: null, error: null },
};
...
export function assertRollbackConfirm(body, expected) {
  ...
  if (!expected?.from) {
    return { ok: false, status: 409, message: 'אין מידע על הגרסאות. אין החזרה.' };
  }
...
export function rollbackSafety(mavConn, confirmUnknown) {
  return assessUpdateSafety(mavConn, { confirmUnknown: confirmUnknown === true });
}
