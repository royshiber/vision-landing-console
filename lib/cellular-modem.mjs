...
export function modemAbsentReasonHe({ statusFileMissing = false } = {}) {
  return statusFileMissing
    ? 'מודם לא מחובר. אין קובץ סטטוס במחשב משימה.'
    : 'מודם לא מחובר.';
}

function absentModem({ statusFileMissing = false, source = 'local' } = {}) {
  return {
    present: false,
    ...
    error: null,
    reason: 'modem_absent',
    reasonHe: modemAbsentReasonHe({ statusFileMissing }),
    source,
...
export function resolveCellularModem({ local = null, companionModem = null } = {}) {
  ...
  const statusFileMissing = companionModem.statusFileMissing === true
    || companionModem.reason === 'status_file_missing';
  ...
  return {
    ...
    reasonHe: present
      ...
      : (companionModem.reasonHe || modemAbsentReasonHe({ statusFileMissing })),
    source: 'companion',
    mock: isMock,
    statusFileMissing: present ? false : statusFileMissing,
  };
}
