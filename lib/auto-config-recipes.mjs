...
const COMPONENT_HINTS = {
  ...
  CompanionLink: {
    ...
    params: [
      ...
      'SR2_POSITION', 'SR2_EXT_STAT', 'SR2_RC_CHAN',
    ],
    queries: ['serial protocol mavlink companion telem2'],
    checks: [
      {
        id: 'companion-telem2',
        title: 'שקע בקר',
        description: 'ודא שהכבל יושב בשקע הטלמטריה השני בבקר.',
        expected: 'שקע מסומן כטלמטריה שתיים',
      },
      {
        id: 'companion-uart',
        title: 'שקע מחשב משימה',
        description: 'ודא שהצד השני יושב בשקע הטורי של מחשב המשימה.',
        expected: 'דופק חיים מגיע לקונסולה',
      },
    ],
  },
  ...
  TelemetryRadio: {
    ...
    params: [
      ...
      'SR1_EXTRA1', 'SR1_POSITION', 'SR1_EXT_STAT',
    ],
    queries: ['serial protocol telem1 radio telemetry stream'],
    checks: [
      {
        id: 'radio-telem1',
        title: 'שקע רדיו',
        description: 'ודא שהרדיו יושב בשקע הטלמטריה הראשון ולא בשקע מחשב המשימה.',
        expected: 'זרם טלמטריה מופיע בקונסולה',
      },
    ],
  },
  ...
  LandingCamera: {
    labelHe: 'מצלמת נחיתה',
    params: [
      'PLND_ENABLED', 'PLND_TYPE',
      'EK3_SRC1_POSXY', 'EK3_SRC1_VELXY',
      'FLOW_TYPE',
    ],
    queries: ['precision landing vision position estimate optical flow'],
    checks: [
      {
        id: 'camera-csi',
        title: 'שקע מצלמה',
        description: 'ודא שמצלמת הנחיתה יושבת בשקע המצלמה של מחשב המשימה.',
        expected: 'אומדן ראייה מגיע',
      },
    ],
  },
 ...
 * Hardware-intent cards expose wiring + expected outcome when present.
 * @returns {Array<{ id: string, labelHe: string, hardwareIntent?: object }>}
 */
export function listComponentTypes() {
  return Object.entries(COMPONENT_HINTS).map(([id, h]) => {
    const intent = getHardwareIntent(id);
    return intent
      ? { id, labelHe: intent.labelHe, hardwareIntent: intent }
      : { id, labelHe: h.labelHe };
  });
}

export { getHardwareIntent, listHardwareIntentComponents };
