...
import { parseRfBaud, parseWorkLink, planWorkLink, probeHttpUrls } from './rf-work-link.mjs';
import { resolveCompanionBaseUrlList } from './companion-api-client.mjs';
import {
...
import { isCompanionRelayLive } from './link-attribution.mjs';
import {
...
export function snapshotDualLink(ctx = {}) {
  ...
  const pathSnap = ctx.companionService?.client?.getPathSnapshot?.() || null;
  const modemReport = pathSnap?.modem || companionModemReport(companion);
  const comm = summarizeCommLinks({
    ...
    companion,
    rc,
    cellularSignal: extractCompanionSignal(companion) || modemReport,
    radioLive,
    ...
    uplinks: normalizeUplinkReport(ctx.networkUplinks),
    uplinkControl: uplinkControlFromCompanion(companion),
    pathProbes: pathSnap,
    modemReport,
    activeTransport: pathSnap?.activeId || null,
  });
...
function radioUsesCompanionRelay(ctx, radioLive) {
  ...
  if (isCompanionRelayLive(radioLive)) return true;
...
function normalizeUplinkReport(raw) {
  if (!raw || typeof raw !== 'object') return null;
  const src = raw.uplinks && typeof raw.uplinks === 'object' ? raw.uplinks : raw;
  return src;
}
...
/** True when turning this uplink off would drop the path the console is using. */
async function uplinkCutWouldLockOut(ctx, which) {
  const client = companionClient(ctx);
  if (!client || typeof client.refreshCompanionPaths !== 'function') return false;
  let snap = null;
  try {
    snap = await client.refreshCompanionPaths({ force: true });
  } catch {
    snap = typeof client.getPathSnapshot === 'function' ? client.getPathSnapshot() : null;
  }
  const roleId = which === 'wifi' ? 'home' : 'cellular';
  const activeId = snap?.activeId || null;
  if (activeId && activeId !== roleId) return false;
  const paths = Array.isArray(snap?.paths) ? snap.paths : [];
  const alternate = paths.some((row) => row && row.ok === true && row.id && row.id !== roleId);
  return !alternate;
}

/** GET /api/v1/network/uplinks only when the client actually has the method. */
export async function refreshNetworkUplinks(ctx = {}) {
  const client = companionClient(ctx);
  if (!client || typeof client.getNetworkUplinks !== 'function') return ctx.networkUplinks ?? null;
  try {
    const body = await client.getNetworkUplinks();
    if (body && typeof body === 'object') ctx.networkUplinks = body;
  } catch {
    /* keep the last report; a missing route is not a fake link */
  }
  return ctx.networkUplinks ?? null;
}

function rememberUplink(ctx, which, enabled, body) {
  const prev = ctx.networkUplinks && typeof ctx.networkUplinks === 'object' ? { ...ctx.networkUplinks } : {};
  if (body && typeof body === 'object' && (body.wifi || body.cellular || (body.uplinks && typeof body.uplinks === 'object'))) {
    const src = body.uplinks && typeof body.uplinks === 'object' ? body.uplinks : body;
    ctx.networkUplinks = { ...prev, ...src };
    return;
  }
  const current = prev[which] && typeof prev[which] === 'object' ? prev[which] : {};
  const extra = body && typeof body === 'object' && (body.up != null || body.enabled != null) ? body : {};
  prev[which] = { ...current, ...extra, enabled: enabled === true };
  ctx.networkUplinks = prev;
}

/**
 * POST companion uplink. home → wifi. Does not close the radio relay or wipe the token.
 * Unsupported capability is a refusal, not a saved console preference.
 */
export async function setCompanionUplink(ctx = {}, { role, enabled } = {}) {
  const which = role === 'cellular' ? 'cellular' : (role === 'wifi' || role === 'home' ? 'wifi' : null);
  if (!which || (enabled !== true && enabled !== false)) {
    return { ok: false, status: 400, messageHe: 'בחרו סלולר או רשת בית', message: 'בחרו סלולר או רשת בית' };
  }
  let control = uplinkControlFromCompanion(snapshotCompanionLinkFromCtx(ctx));
  if (!control && typeof ctx.probeUplinkControl === 'function') {
    try { control = (await ctx.probeUplinkControl()) === true; } catch { control = false; }
  }
  if (!control) {
    return {
      ok: false,
      status: 409,
      uplinkControl: false,
      messageHe: UPLINK_UNSUPPORTED_HE,
      message: UPLINK_UNSUPPORTED_HE,
    };
  }
  const client = companionClient(ctx);
  const poster = typeof ctx.setNetworkUplink === 'function'
    ? ctx.setNetworkUplink
    : (typeof client?.setNetworkUplink === 'function' ? client.setNetworkUplink.bind(client) : null);
  if (!poster) {
    return {
      ok: false,
      status: 409,
  ...
  if (enabled === false && await uplinkCutWouldLockOut(ctx, which)) {
    return {
      ok: false,
      status: 409,
      uplinkControl: true,
      reason: 'path_lock',
      messageHe: UPLINK_PATH_LOCK_HE,
      message: UPLINK_PATH_LOCK_HE,
    };
  }
  try {
    const body = await poster(which, { enabled: enabled === true });
    rememberUplink(ctx, which, enabled === true, body);
    return { ok: true, uplinkControl: true, which, enabled: enabled === true };
  } catch (err) {
    const status = Number(err?.status) || 502;
    if (status === 409) {
      const messageHe = uplinkRefusalHe(err?.body);
      return { ok: false, status: 409, uplinkControl: true, messageHe, message: messageHe };
    }
    return {
      ok: false,
      status,
      uplinkControl: true,
      messageHe: UPLINK_REJECTED_HE,
      message: UPLINK_REJECTED_HE,
    };
  }
}
...
const WORK_LINK_KEY = 'workLink.v1';

export function readWorkLink(db) {
  const raw = db ? getConfig(db, WORK_LINK_KEY, null) : null;
  const obj = raw && typeof raw === 'object' ? raw : {};
  return {
    mode: parseWorkLink(obj.mode),
    serialPort: typeof obj.serialPort === 'string' ? obj.serialPort : '',
    baudRate: parseRfBaud(obj.baudRate),
  };
}

export function writeWorkLink(db, value) {
  const next = {
    mode: parseWorkLink(value?.mode),
    serialPort: String(value?.serialPort || '').trim(),
    baudRate: parseRfBaud(value?.baudRate),
  };
  if (db) setConfig(db, WORK_LINK_KEY, next);
  return next;
}

export async function connectRfSerial(ctx, { serialPort, baudRate } = {}) {
  const db = ctx?.db;
  const port = String(serialPort || '').trim();
  if (!port) return { ok: false, message: 'בחרו פורט' };
  if (!db) return { ok: false, message: 'אין מאגר' };
  const baud = parseRfBaud(baudRate);
  deactivateRfSerialConnections();
  const name = `RF ${port}`;
  const id = insertProfile(db, {
    name,
  ...
  try {
    ...
    try { db.prepare('DELETE FROM connections WHERE id = ?').run(id); } catch { /* ignore */ }
    return { ok: false, message: err?.message || 'פתיחת הפורט נכשלה' };
  }
  db.prepare(`UPDATE connections SET active = 1, last_connected = datetime('now') WHERE id = ?`).run(id);
  setCommandLinkId(id);
  return { ok: true, id, role: 'rf', status: getConnectionStatus(id) };
}

export async function applyWorkLink(ctx, body = {}, { fetchImpl } = {}) {
  const prev = readWorkLink(ctx?.db);
  const mode = parseWorkLink(body?.mode ?? prev.mode);
  const serialPort = body?.serialPort != null ? String(body.serialPort).trim() : prev.serialPort;
  const baudRate = body?.baudRate != null ? parseRfBaud(body.baudRate) : prev.baudRate;
  const stored = writeWorkLink(ctx?.db, { mode, serialPort, baudRate });
  const urls = resolveCompanionBaseUrlList(ctx?.env || process.env);
  const httpResults = await probeHttpUrls(urls, fetchImpl || fetch);
  const plan = planWorkLink({ mode: stored.mode, httpResults, serialPort: stored.serialPort });
  let serial = null;
  if (plan.connectSerial) {
    serial = await connectRfSerial(ctx, { serialPort: stored.serialPort, baudRate: stored.baudRate });
    if (!serial.ok) return { ok: false, ...plan, ...stored, serial, httpResults };
  } else if (plan.disconnectSerial) {
    deactivateRfSerialConnections();
  }
  return { ok: true, ...plan, ...stored, serial, httpResults };
}

export function sendRfCompanionAction(body = {}) {
  const kind = String(body?.kind || '');
  const conn = findRfSerialConnection();
  if (!conn) return { ok: false, message: 'אין קישור רדיו' };
  if (kind === 'uplink') {
    const cellular = body.role === 'cellular';
    conn.sendCompanionCommand(RF_CMD_UPLINK, [cellular ? 1 : 0, body.enabled === true ? 1 : 0]);
    return { ok: true, kind };
  }
  if (kind === 'gimbal') {
    const action = String(body.action || '');
    if (action === 'rate') {
      conn.sendCompanionCommand(RF_CMD_GIMBAL, [1, Number(body.yaw) || 0, Number(body.pitch) || 0]);
    } else if (action === 'zoom') {
      conn.sendCompanionCommand(RF_CMD_GIMBAL, [2, Number(body.zoom) || 0]);
    } else if (action === 'mode') {
      conn.sendCompanionCommand(RF_CMD_GIMBAL, [3, body.mode === 'lock' ? 1 : 0]);
    } else {
  ...
  if (kind === 'track') {
    conn.sendCompanionCommand(RF_CMD_TRACK, [body.enabled === true ? 1 : 0]);
    return { ok: false, disabled: true, message: 'מעקב עדיין כבוי' };
  }
  return { ok: false, message: 'פעולה לא נתמכת' };
}
