...
export async function setCompanionUplink(ctx = {}, { role, enabled } = {}) {
  ...
  const poster = typeof ctx.postNetworkUplink === 'function'
    ? ctx.postNetworkUplink
    : (typeof companionClient(ctx)?.postNetworkUplink === 'function'
      ? (name, on) => companionClient(ctx).postNetworkUplink(name, on)
      : null);
  if (!poster) {
    return {
      ok: false,
  ...
  try {
    const body = await poster(which, enabled === true);
