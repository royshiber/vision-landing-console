...
export async function connectLink(ctx, { role, type, host, port, serialPort, baudRate, via, force, preset, simulatorPreset } = {}) {
  const shaped = applySimulatorPreset({ type, host, port, serialPort, via, preset, simulatorPreset });
  type = shaped.type;
  host = shaped.host;
  port = shaped.port;
  serialPort = shaped.serialPort;
  via = shaped.via;
  const db = ctx.db;
  ...
  try {
    ...
    return { ok: false, message: simMessage || err?.message || 'הפעלת חיבור נכשלה', state: 'error' };
  }
  db.prepare(`UPDATE connections SET active = 1, last_connected = datetime('now') WHERE id = ?`).run(id);
