...
function buildDescriptions() {
  const he = {
    ...
    AHRS_EKF_TYPE: 'איזה מסנן ניווט משמש להטסה',
    ...
    PLND_STRICT: 'חומרת הנחיתה המדויקת',
    ...
    TECS_TIME_CONST: 'קבוע הזמן של בקר המהירות',
    ...
    FS_GCS_ENABL: 'הפעלת כשל בטוח מול תחנת הקרקע',
    THR_FAILSAFE: 'הפעלת כשל מצערת ושלט',
    THR_FS_VALUE: 'ערך מצערת שמפעיל כשל',
    BATT_FS_LOW_ACT: 'פעולה בסוללה נמוכה',
    BATT_FS_CRT_ACT: 'פעולה בסוללה קריטית',
    FS_EKF_THRESH: 'סף כשל של מסנן הניווט',
    ...
    ARMING_RUDDER: 'חימוש בהגאי הכיוון',
  ...
  for (let n = 1; n <= 8; n += 1) {
    ...
    he[`SERIAL${n}_BAUD`] = `קצב ${port}`;
    he[`SERVO${n}_FUNCTION`] = `תפקיד יציאת סרוו ${n}`;
  }
  for (let n = 1; n <= 4; n += 1) {
    he[`RC${n}_MIN`] = `סף תחתון של ערוץ שלט ${n}`;
    he[`RC${n}_MAX`] = `סף עליון של ערוץ שלט ${n}`;
