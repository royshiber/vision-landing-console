...
import { fcTelemetryFields, sseSysId } from './mavlink-connection.mjs';
import { detectSimulatorVehicle } from './sim-vehicle.mjs';
...
export function buildSseMavlinkSnapshot(mavConn) {
  ...
  const fc = fcTelemetryFields(mavConn, now);
  const linkUp = mavConn.connected === true || mavConn.listening === true;
  const sim = typeof mavConn.simulatorDetection === 'function'
    ? mavConn.simulatorDetection()
    : detectSimulatorVehicle({
      simulatorPreset: mavConn.simulatorPreset === true,
      preset: mavConn.preset,
      name: mavConn.name,
      autopilotVersion: mavConn.autopilotVersion,
      params: mavConn.params,
    });
  const simulator = linkUp && sim.simulator === true;
  return {
    connected: mavConn.connected === true,
    listening: mavConn.listening === true,
    simulator,
    simulatorReason: simulator ? sim.reason : null,
    id: mavConn.id ?? null,
    linkRole: mavConn.linkRole || 'radio',
    heartbeatCount: Number(mavConn.heartbeatCount) || 0,
    sysId: sseSysId(mavConn.sysId),
    ...
    batteryV: fc.batteryV,
    batteryPct: fc.batteryPct,
    batteryCurrentA: fc.batteryCurrentA,
    batteryConsumedMah: fc.batteryConsumedMah,
    fcLoadPct: fc.fcLoadPct,
    fcMemFreeKB: fc.fcMemFreeKB,
    fcMemPct: fc.fcMemPct,
    fcTempC: fc.fcTempC,
    fcMcuVoltageV: fc.fcMcuVoltageV,
    powerVccV: fc.powerVccV,
    gpsFixType: fc.gpsFixType,
    gpsSats: fc.gpsSats,
    rcChannels: fc.rcChannels,
    batteryAgeMs: fc.batteryAgeMs,
    fcLoadAgeMs: fc.fcLoadAgeMs,
    fcMemAgeMs: fc.fcMemAgeMs,
    fcTempAgeMs: fc.fcTempAgeMs,
    gpsAgeMs: fc.gpsAgeMs,
    rcAgeMs: fc.rcAgeMs,
    map: mapTelemetry,
