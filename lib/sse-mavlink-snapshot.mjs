...
import { isVehicleFlying } from './arm-disarm.mjs';
...
export function buildSseMavlinkSnapshot(mavConn) {
  ...
  return {
    ...
    flying: isVehicleFlying(mavConn, now),
    landedState: Number.isInteger(mavConn.lastLandedState) ? mavConn.lastLandedState : null,
    landedStateAgeMs: Number.isFinite(mavConn.lastLandedStateAt) ? Math.max(0, now - mavConn.lastLandedStateAt) : null,
