...
async function collectStatusBundle(client) {
  ...
  const [status, visionResult, navigationEstimate, diagnostics, version, config, policy, policyPreview, health] = await Promise.all([
    client.getStatus(),
    ...
    client.getHealth ? client.getHealth().catch(() => null) : Promise.resolve(null),
  ]);
  return {
    ...status,
    visionResult,
