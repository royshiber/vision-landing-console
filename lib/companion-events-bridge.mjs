...
export function companionMappedToSseOverlay(mapped, meta) {
  ...
  return {
    ...
    companion: {
      ...
      extras: unreachable ? null : (m.extras || null),
      opticalNav: unreachable ? null : m.opticalNav,
...
export function mergeTelemetryWithCompanion(base, overlay) {
  ...
  if (overlay.hasSnapshot || overlayJetson) {
    ...
    payload.vision.navLat = preserveFiniteIfNull(base.vision?.navLat, overlay.vision?.navLat);
    payload.vision.navLon = preserveFiniteIfNull(base.vision?.navLon, overlay.vision?.navLon);
    payload.opticalNav = overlay.companion?.opticalNav || overlay.vision?.opticalNav || base.opticalNav || null;
...
export function statusBundleFromLegacyHealth(health) {
  ...
  return {
    ...
    vision: h.vision && typeof h.vision === 'object' ? h.vision : undefined,
    optical_nav: h.optical_nav && typeof h.optical_nav === 'object' ? h.optical_nav : undefined,
    landing: h.landing && typeof h.landing === 'object' ? h.landing : undefined,
    video: h.video && typeof h.video === 'object' ? h.video : undefined,
    extras: h.extras && typeof h.extras === 'object' ? h.extras : undefined,
    modem: h.modem && typeof h.modem === 'object' ? h.modem : undefined,
...
export async function collectStatusBundle(client) {
  ...
  const [visionResult, navigationEstimate, opticalNav, diagnostics, version, config, policy, policyPreview] = await Promise.all([
    ...
    client.getStatusOpticalNav ? client.getStatusOpticalNav().catch(() => null) : Promise.resolve(null),
  ...
  return {
    ...
    visionResult,
    navigationEstimate,
    optical_nav: status.optical_nav || opticalNav,
    diagnostics,
...
export function createCompanionEventBridge(opts) {
  ...
  async function refresh() {
    if (typeof client?.refreshCompanionPaths === 'function') {
      try { await client.refreshCompanionPaths(); } catch { /* keep the last path snapshot */ }
    }
    try {
