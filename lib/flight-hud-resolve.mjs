...
export const FLIGHT_HUD_CATALOG = [
  { key: 'mavlink.airspeed',      label: 'מהירות אוויר',       unit: 'm/s', mav: 'VFR_HUD.airspeed' },
  { key: 'mavlink.groundspeed',   label: 'מהירות קרקעית',      unit: 'm/s', mav: 'VFR_HUD.groundspeed' },
  { key: 'mavlink.climbRate',     label: 'מהירות אנכית',       unit: 'm/s', mav: 'VFR_HUD.climb' },
  { key: 'mavlink.altitude',      label: 'גובה',               unit: 'm', mav: 'VFR_HUD.alt' },
  { key: 'mavlink.heading',       label: 'כיוון אף',           unit: '°', mav: 'VFR_HUD.heading' },
  { key: 'mavlink.flightMode',    label: 'מוד טיסה',           unit: '', mav: 'HEARTBEAT.custom_mode' },
  { key: 'mavlink.armed',         label: 'מצב ARM',            unit: '', mav: 'HEARTBEAT.base_mode' },
  { key: 'mavlink.batteryV',      label: 'מתח סוללה',          unit: 'V', mav: 'SYS_STATUS.voltage_battery' },
  { key: 'mavlink.batteryPct',    label: 'טעינת סוללה',        unit: '%', mav: 'SYS_STATUS.battery_remaining' },
  { key: 'mavlink.fcLoadPct',     label: 'עומס FC',            unit: '%', mav: 'SYS_STATUS.load' },
  { key: 'mavlink.rollDeg',       label: 'רול',                unit: '°', mav: 'ATTITUDE.roll' },
  { key: 'mavlink.pitchDeg',      label: 'פיץ׳',              unit: '°', mav: 'ATTITUDE.pitch' },
  { key: 'mavlink.gpsFixType',    label: 'Fix GPS',            unit: '', mav: 'GPS_RAW_INT.fix_type' },
  { key: 'mavlink.gpsSats',       label: 'לוויינים GPS',       unit: '', mav: 'GPS_RAW_INT.satellites_visible' },
...
const SYNONYMS = {
  ...
  'mavlink.fcLoadPct':      ['fc load', 'cpu fc', 'עומס fc', 'sys_status.load'],
  ...
  'vision.confidence':      ['conf', 'confidence', 'ביטחון', 'ויזן', 'נחיתה', 'landing'],
  'mission.link':           ['link', 'קישור', 'rssi', 'חיבור', 'mavlink'],
  'mission.gpsVisionDelta': ['הפרש', 'ראייה', 'ראיה', 'delta', 'gps vision', 'vision delta', 'הפרש ראייה'],
...
function scoreEntry(textNorm, entry) {
  ...
  if (mav && (textNorm === mav || textNorm.includes(mav))) return 96;
  ...
  if (mav) {
    ...
    if (mavField.length >= 2 && textNorm.includes(mavField)) score += 28;
    if (textNorm.includes(mav.split('.')[0])) score += 10;
  }
...
/**
 * Mission data-tile picker: chips from free text. Local catalog only. Never invents GPS.
 * @param {string} text
 * @returns {{ chips: typeof FLIGHT_HUD_CATALOG, hint: string, exact: (typeof FLIGHT_HUD_CATALOG)[0] | null }}
 */
export function suggestMissionDataFields(text) {
  const ranked = rankHudParamMatches(text);
  const hintRefine = 'לא זוהה במדויק. נסחו מחדש או בחרו אפשרות.';
  if (!String(text || '').trim()) {
    return { chips: FLIGHT_HUD_CATALOG.slice(0, 6), hint: 'כתבו מה להציג, או בחרו אפשרות.', exact: null };
  }
  if (ranked.length === 0) {
    return { chips: FLIGHT_HUD_CATALOG.slice(0, 8), hint: hintRefine, exact: null };
  }
  const local = resolveHudParamLocally(text);
  if (local.kind === 'match') {
    const rest = ranked.filter((r) => r.entry.key !== local.key).slice(0, 4).map((r) => r.entry);
    const picked = {
      key: local.key,
      label: local.label,
      unit: local.unit,
      mav: local.mav || FLIGHT_HUD_CATALOG.find((e) => e.key === local.key)?.mav || '',
    };
    return {
      chips: [picked, ...rest],
  ...
  if (local.kind === 'ambiguous') {
    return { chips: local.options, hint: local.hint, exact: null };
  }
  const chips = ranked.slice(0, 6).map((r) => r.entry);
  return { chips, hint: hintRefine, exact: null };
}
