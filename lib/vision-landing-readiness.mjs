...
export function resolveRunwayDetectState({
  ...
  const label = String(o.label || o.class || o.class_name || '').toLowerCase();
  const hasRunwayLabel = /runway|strip/.test(label);
  const hasApi = detected != null || hasDetections || validity !== '' || hasRunwayLabel;
  if (!hasApi) return 'unknown';
  if (detected === true || hasDetections || hasRunwayLabel || validity === 'valid' || validity === 'degraded') {
    return 'detected';
  }
