...
import { annotatedVideoAvailability, ANNOTATED_VIDEO_REASON_HE_MISSION } from './dual-link.mjs';
import {
  buildCameraInstallChecklist,
  cameraInstallReadinessRow,
...
function cameraRow(state) {
  ...
  if (state === 'absent') {
    return row('camera_vision', state, 'צינור ראייה', 'חסר', 'אין דיווח מצלמה', 'bad');
  }
  return row('camera_vision', 'unknown', 'צינור ראייה', 'לא ידוע', 'אין דיווח מצלמה', 'off');
}
...
function runwayDetectRow(state) {
  ...
  if (state === 'absent') {
    return row('runway_detect', state, 'זיהוי מסלול בגמר', 'חסר', 'גלאי כבוי במחשב משימה', 'bad', {
      successCriterion: true,
    });
  }
  if (state === 'not_implemented') {
    return row('runway_detect', state, 'זיהוי מסלול בגמר', 'אין גלאי', 'אין גלאי במחשב משימה', 'bad', {
      successCriterion: true,
    });
  }
  return row('runway_detect', 'unknown', 'זיהוי מסלול בגמר', 'לא ידוע', 'אין דיווח זיהוי', 'off', {
    successCriterion: true,
  });
}
...
function plndRow(honesty) {
  ...
  if (snapshot.state === 'present') {
    ...
    return row(
      ...
      'קיים',
      incomplete ? 'חסרים מפתחות. לא חוסם את הניסוי.' : '',
      incomplete ? 'warn' : 'ok',
      extra,
    );
  }
 ...
 * No modem → modem_absent. No annotated stream → stream_absent.
 * Radio / overlay.available never invent a live path.
 */
export function resolveAnnotatedVideoHonesty({
  ...
  const claimedStream = streamPresent === true
    || overlay?.streamPresent === true
    || overlay?.annotatedStream === true;
  const cell = cellular
    || overlay?.cellular
    || 'disconnected';
  return annotatedVideoAvailability({
    cellular: cell,
    modemPresent: true,
    streamPresent: claimedStream,
  });
}

function videoRow({ video, cellular, modemPresent, streamPresent } = {}) {
  const gate = resolveAnnotatedVideoHonesty({ video, cellular, modemPresent, streamPresent });
  const shortReason = ANNOTATED_VIDEO_REASON_HE_MISSION[gate.reason] || '';
  const note = gate.reason === 'modem_absent'
    ? 'לא נדרש. מודם סלולר לא מחובר.'
    : gate.available
      ? 'לא נדרש.'
      : `לא נדרש. ${shortReason}`.trim();
  return row(
    'annotated_video',
    'later',
    'סימון על הווידאו',
    ...
    'later',
    {
      ...
      streamPresent: gate.streamPresent === true,
...
export function buildVisionLandingReadiness(input = {}) {
  ...
  const cameraInstall = buildCameraInstallChecklist({
    operator: input.cameraInstallOperator,
    companion,
    vision,
    video: videoOverlay,
    extras,
    opticalNav: obj(overlay.optical_nav) || obj(companion.optical_nav) || obj(input.opticalNav),
  });
  const rows = [
    jetsonRow(jetson),
    fcRow(fc),
    cameraRow(camera),
    cameraInstallReadinessRow(cameraInstall),
