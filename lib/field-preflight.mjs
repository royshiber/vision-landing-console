 ...
 * ARM / DISARM stay blocked. LAND / RTL after an open Ask voice session.
 */

import { summarizeCommLinks, qualityFromCompanionSignal, EMPTY_QUALITY } from './comm-links.mjs';
import { annotatedStreamPresent, DEFAULT_CELLULAR_ENDPOINT } from './dual-link.mjs';
import {
...
export const ARM_LAND_TOKENS = Object.freeze(['ARM', 'DISARM']);
...
function annotatedVideoRow({ video, cellular, modemPresent, streamPresent } = {}) {
  const gate = resolveAnnotatedVideoHonesty({ video, cellular, modemPresent, streamPresent });
  const available = gate.available === true;
  const reason = gate.reason || (modemPresent ? 'stream_absent' : 'modem_absent');
  const short = reason === 'modem_absent'
    ? 'אין שידור. מודם לא מחובר.'
    : reason === 'cellular_disconnected'
      ? 'אין שידור. סלולר מנותק.'
      : reason === 'stream_absent'
        ? 'אין שידור. אין זרם מסומן.'
        : 'שידור סלולר ממחשב משימה.';
  return row('annotated_video', {
    state: available ? 'available' : reason,
    tone: available ? 'ok' : 'absent',
    nameHe: 'ראייה מסומנת',
    stateHe: available ? 'זמין' : 'אין שידור',
    missingHe: available ? '' : short,
    requiredForExperiment1: false,
    extra: {
      path: 'cellular',
      available,
      reason,
      reasonHe: gate.reasonHe || short,
      neverRadio: true,
...
function askGoRow(state) {
  if (state === 'on') {
    return row('ask_go', {
      ...
      nameHe: 'הפעלת קול',
      stateHe: 'מופעל',
      missingHe: 'שיחת הקול פתוחה. אחרי ההפעלה, נחיתה ושינוי מצב מתבצעים בלי אישור נוסף. חימוש נשאר חסום, ושינוי פרמטר עדיין דורש אישור.',
    });
  }
  if (state === 'off') {
    return row('ask_go', {
      ...
      nameHe: 'הפעלת קול',
      stateHe: 'כבוי',
      missingHe: 'שיחת קול כבויה. נחיתה דורשת פתיחה. חימוש חסום. פרמטר דורש אישור.',
    });
  }
  return row('ask_go', {
    state: 'unknown',
    ...
    nameHe: 'הפעלת קול',
    stateHe: 'לא ידוע',
    missingHe: 'אין סשן. חימוש חסום. נחיתה דורשת פתיחת שיחת קול.',
  });
}
...
function armLandBlockedRow() {
  return row('arm_land_blocked', {
    ...
    nameHe: 'חימוש ונטרול',
    stateHe: 'חסום',
    missingHe: 'חימוש ונטרול חסומים תמיד. נחיתה אחרי שיחת קול.',
    requiredForExperiment1: true,
    extra: {
...
export function buildFieldPreflight(input = {}) {
  ...
  const rows = [
    ...
    annotatedVideoRow({
      video: input.video || overlay.video,
      cellular: input.cellular,
      modemPresent,
      streamPresent: input.streamPresent === true
        || annotatedStreamPresent(companion)
        || annotatedStreamPresent(overlay),
    }),
    plndObserveRow(plnd),
    askGoRow(askGo),
    armLandBlockedRow(),
