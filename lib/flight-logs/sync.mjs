...
export function publicFlightLogsStatus(ctx) {
  ...
  let messageHe = null;
  if (!cfg.configured) messageHe = NOT_CONFIGURED_HE;
...
export async function syncFlightLogs(ctx, { reason: _reason = 'manual' } = {}) {
  ...
  try {
    ...
    for (const vehicle of vehicles) {
      let indexes = [];
      try {
        indexes = await provider.listIndex(vehicle);
      } catch (err) {
...
export async function openFlightBundle(ctx, uid) {
  const cfg = flightLogsConfig();
  if (!cfg.configured) {
    const err = new Error(NOT_CONFIGURED_HE);
    err.status = 409;
    err.messageHe = NOT_CONFIGURED_HE;
    throw err;
  }
...
export function listFlightLogs(ctx, query) {
  const cfg = flightLogsConfig();
  if (!cfg.configured) {
    return { ok: true, configured: false, flights: [], messageHe: NOT_CONFIGURED_HE };
  }
