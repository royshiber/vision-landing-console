...
import { cloudGapHe } from './messages.mjs';
...
export function flightLogsConfig(env = process.env) {
  const raw = String(env.FLIGHT_LOGS_MODE ?? '').trim().toLowerCase();
  const explicitOff = raw === 'off' || raw === '0' || raw === 'false' || raw === 'no';
  let mode = raw === 'mock' || raw === 'cloud' ? raw : 'off';
  ...
  if (mode === 'off' && !explicitOff && cloudReady) mode = 'cloud';
  const configured = mode === 'mock' || (mode === 'cloud' && cloudReady);
  const gapHe = configured ? null : cloudGapHe({
    endpoint, region, bucket, keyId, secret: appKey, explicitOff, cloudReady,
  });
