...
export function flightLogsConfig(env = process.env) {
  ...
  const endpoint = String(env.FLIGHT_CLOUD_ENDPOINT || '').trim();
  const region = String(env.FLIGHT_CLOUD_REGION || '').trim();
  const bucket = String(env.FLIGHT_CLOUD_BUCKET || '').trim();
  const keyId = String(env.FLIGHT_CLOUD_KEY_ID || '').trim();
  const appKey = String(env.FLIGHT_CLOUD_APP_KEY || '').trim();
  const cloudReady = Boolean(endpoint && region && bucket && keyId && appKey);
  const configured = mode === 'mock' || (mode === 'cloud' && cloudReady);
  const vehicles = String(env.FLIGHT_CLOUD_VEHICLES || '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  const prefix = String(env.FLIGHT_CLOUD_PREFIX || 'v1').replace(/^\/+|\/+$/g, '') || 'v1';
