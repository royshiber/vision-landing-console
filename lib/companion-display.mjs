...
export function flattenCompanionConfig(cfg) {
  ...
  const groups = [
    { group: 'JETSON / COMPANION', key: 'static', tier: CONFIG_TIER.READ_ONLY },
    { group: 'SYSTEM', key: 'read_only', tier: CONFIG_TIER.READ_ONLY },
    { group: 'SYSTEM', key: 'runtime', tier: CONFIG_TIER.RUNTIME },
    { group: 'NAVIGATION / FLIGHT', key: 'flight_critical', tier: CONFIG_TIER.FLIGHT_CRITICAL },
    { group: 'CAMERA', key: 'camera_extrinsics', tier: CONFIG_TIER.READ_ONLY },
    { group: 'VISION', key: 'aruco', tier: CONFIG_TIER.READ_ONLY },
    { group: 'VISION', key: 'vision', tier: CONFIG_TIER.RUNTIME },
  ];
  const rows = [];
  for (const g of groups) {
    const runtimeEditable = g.tier === CONFIG_TIER.RUNTIME;
...
const RUNTIME_PATCH_GROUPS = Object.freeze(['runtime', 'vision']);

export function coerceCompanionConfigValue(previous, raw) {
  ...
  if (typeof previous === 'string') {
    ...
    if ((trimmed.startsWith('[') || trimmed.startsWith('{')) && (text.startsWith('[') || text.startsWith('{'))) {
      try {
...
/**
 * Build a RUNTIME-only PATCH body from flattened rows + edited values.
 * Never includes FLIGHT_CRITICAL / READ_ONLY keys.
 */
export function buildRuntimeConfigPatch(rows, valuesByKey) {
  const patch = {};
  const list = Array.isArray(rows) ? rows : [];
  const values = valuesByKey && typeof valuesByKey === 'object' ? valuesByKey : {};
  for (const row of list) {
    if (!row || row.tier !== CONFIG_TIER.RUNTIME || row.editable !== true) continue;
    const key = String(row.key || '');
    const dot = key.indexOf('.');
    if (dot <= 0) continue;
    const group = key.slice(0, dot);
    const field = key.slice(dot + 1);
    if (!RUNTIME_PATCH_GROUPS.includes(group) || !field) continue;
    if (!Object.prototype.hasOwnProperty.call(values, key)) continue;
    if (!patch[group]) patch[group] = {};
    patch[group][field] = coerceCompanionConfigValue(row.value, values[key]);
  }
  return patch;
}

/**
 * Drop FLIGHT_CRITICAL / READ_ONLY keys from a runtime PATCH body.
 * Nested `runtime` + `vision` win; otherwise flat keys pass through minus forbidden groups.
 */
export function sanitizeRuntimeConfigPatch(body) {
  const src = body && typeof body === 'object' && !Array.isArray(body) ? body : {};
  const out = {};
  if (src.runtime && typeof src.runtime === 'object' && !Array.isArray(src.runtime)) {
    out.runtime = { ...src.runtime };
  }
  if (src.vision && typeof src.vision === 'object' && !Array.isArray(src.vision)) {
    out.vision = { ...src.vision };
  }
  if (out.runtime || out.vision) return out;
  const rest = {};
  for (const [k, v] of Object.entries(src)) {
    if (k === 'flight_critical' || k === 'static' || k === 'read_only' || k === 'camera_extrinsics' || k === 'aruco') {
      continue;
    }
    rest[k] = v;
  }
