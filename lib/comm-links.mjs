...
import { attributeLinkCards } from './link-attribution.mjs';
...
export const UPLINK_PATH_LOCK_HE = 'אי אפשר לכבות את הקישור שבשימוש, כי אין עוד דרך פעילה למחשב המשימה.';
...
function buildRow(id, extra) {
  ...
  return {
    ...
    ...(typeof extra.errorHe === 'string' ? { errorHe: extra.errorHe } : {}),
  };
}
...
export function summarizeCommLinks({
  ...
  const attributed = attributeLinkCards({
    radioDisplay,
    cellDisplay,
    homeDisplay,
    radioLive,
    ...
    pathProbes,
    modemReport,
    activeId: activeTransport,
    homeEnabled: enabled.home,
    cellEnabled: enabled.cellular,
  });
  const radioCard = attributed.radioDisplay;
  const cellCard = attributed.cellDisplay;
  const homeCard = attributed.homeDisplay;
  const homeLatency = attributed.homeRttMs != null
    ? qualityFromHttpRtt(attributed.homeRttMs)
    : null;
  const cellLatency = attributed.cellRttMs != null
    ? qualityFromHttpRtt(attributed.cellRttMs)
    : null;

  const radioHint = enabled.radio === false
    ? DISABLED_HE
    : (attributed.relaySocket
      ? COMM_LINK_META.radio.hintHe
      : (radioCard.connected && dual.active === 'radio'
        ? 'הכל חוץ מווידאו · פעיל לפקודות'
        : COMM_LINK_META.radio.hintHe));

  const displays = { radio: radioCard, cellular: cellCard, home: homeCard };
  const pillLabelHe = attributed.activeId === 'home' && homeCard.connected
    ? 'מחובר · רשת בית'
    : (attributed.activeId === 'cellular' && cellCard.connected
      ? 'מחובר · סלולר'
      : pillFromDisplays({ ...displays, active: dual.active }));
  const rows = [
    buildRow('cellular', {
      ...cellCard,
      enabled: cellLink ? cellLink.enabled !== false : enabled.cellular,
      uplinkControl: control,
      actionOn: control && cellCard.sessionOpen === true,
      errorHe: attributed.cellErrorHe,
      hintHe: [
        attributed.cellHintHe,
        cellLatency?.known ? cellLatency.tooltipHe : null,
      ].filter(Boolean).join(' · ')
        || (cellLink
          ? CELLULAR_UPLINK_HINT_HE
          : (cellCard.statusHe === 'אין מודם'
            ? 'מודם לא מחובר'
            : (enabled.cellular === false ? DISABLED_HE : CELLULAR_UPLINK_HINT_HE))),
      quality: {
        ...(cellBars || cellLatency || cellLiveQuality),
        ...(cellCaption.networkLabel ? { networkLabel: cellCaption.networkLabel } : {}),
        ...(cellCaption.operator ? { operator: cellCaption.operator } : {}),
      },
      active: attributed.activeId === 'cellular' && cellCard.connected === true,
    }),
    buildRow('radio', {
      ...radioCard,
      enabled: enabled.radio,
      actionOn: enabled.radio !== false && radioCard.sessionOpen === true,
      hintHe: radioHint,
      quality: attributed.relaySocket ? { ...EMPTY_QUALITY } : radioQuality,
      active: !attributed.relaySocket && dual.active === 'radio' && radioCard.connected === true,
    }),
    buildRow('home', {
      ...homeCard,
      enabled: wifiLink ? wifiLink.enabled !== false : enabled.home,
      uplinkControl: control,
      actionOn: control && homeCard.sessionOpen === true,
      errorHe: attributed.homeErrorHe,
      hintHe: wifiLink && wifiLink.enabled === false
        ? DISABLED_HE
        : (enabled.home === false && !wifiLink
          ? DISABLED_HE
          : (homeLatency?.known
            ? homeLatency.tooltipHe
            : (homeCard.connected
              ? COMM_LINK_META.home.hintHe
              : (companion?.hint_he || COMM_LINK_META.home.hintHe)))),
      quality: homeLatency?.known
        ? homeLatency
        : (wifiQuality.known ? wifiQuality : homeQuality(companion, homeRttMs)),
      active: attributed.activeId === 'home' && homeCard.connected === true,
    }),
    buildRow('rc', {
