...
export function uplinkRefusalHe(body) {
  ...
  const he = src.reason_he || src.message_he || src.messageHe || src.status_he;
...
function uplinkSignalQuality(link) {
  ...
  const rssiDbm = finiteNumber(link.signal_dbm ?? link.rssi_dbm);
  return qualityFromCompanionSignal({
    rsrp: link.rsrp ?? link.rsrp_dbm,
    rssi_dbm: rssiDbm,
    rssi: rssiDbm == null ? link.rssi : undefined,
    rsrq: link.rsrq,
    sinr: link.sinr,
  }, 'עוצמת קליטה');
}
...
export function summarizeCommLinks({
  ...
  const rows = [
    buildRow('cellular', {
      ...
      quality: cellUplinkQuality.known ? cellUplinkQuality : (cellSignal.known ? cellSignal : cellLiveQuality),
