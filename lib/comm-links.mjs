...
/** Fresh MAVLink heartbeat or companion health. Older than this is not connected. */
export const HEARTBEAT_LIVE_MS = 5000;
...
/** Heartbeat age → bars. Under 1s = 4, 2s = 3, 3.5s = 2, 5s = 1, else 0. No invented percent. */
export function qualityFromHeartbeatAge(ageMs, { rateHz = null, sourceHe = 'לפי זמן דופק' } = {}) {
  ...
  else if (age < 3500) bars = 2;
  else if (age < 5000) bars = 1;
...
export function qualityFromCompanionSignal(raw, sourceHe = 'עוצמת אות מודם') {
  ...
  if (obj.rssi != null && obj.rssi !== '') {
    ...
    if (rssiN != null && rssiN < 0) {
      return knownBars(barsFromRssiDbm(rssiN), 'עוצמת קליטה', `עוצמת קליטה ${rssiN} דציבל${extra}`);
    }
  ...
  if (rsrq != null) {
    return knownBars(barsFromRsrqDb(rsrq), 'איכות קליטה', `איכות קליטה ${rsrq}`);
  }
  if (sinr != null) {
    return knownBars(barsFromSinrDb(sinr), 'יחס רעש', `יחס רעש ${sinr}`);
  }
  return { ...EMPTY_QUALITY };
}
...
/** Listening with no frames is waiting, never connected. Stale heartbeat is degraded. */
export function deriveMavlinkDisplay(live, { disabled = false } = {}) {
  ...
  const frames = Number(live.framesRx) || 0;
  const hb = Number(live.heartbeatCount) || 0;
  const hasTraffic = hb > 0 || frames > 0;
  const fresh = hasTraffic && age != null && age >= 0 && age < HEARTBEAT_LIVE_MS;
  const stale = hasTraffic && age != null && age >= HEARTBEAT_LIVE_MS;
  const open = live.connected === true || live.listening === true;
  if (fresh) {
...
/** Fresh companion health plus an FC heartbeat is connected. Reachable without FC is degraded. */
export function deriveHomeDisplay(companion, { disabled = false, now = Date.now() } = {}) {
  ...
  const fcLive = companion?.fc === 'heartbeat' || companion?.fc_heartbeat === true;
  if (!fresh) {
...
export function summarizeCommLinks({
  ...
  const homeDisplay = deriveHomeDisplay(companion, { disabled: enabled.home === false });
  ...
  const rows = [
    buildRow('cellular', {
      ...cellDisplay,
      enabled: enabled.cellular,
      actionOn: enabled.cellular === true && !cellAbsent,
      hintHe: cellAbsent
        ? 'מודם לא מחובר'
        : (enabled.cellular === false ? DISABLED_HE : CELLULAR_UPLINK_HINT_HE),
      quality: cellSignal.known ? cellSignal : cellLiveQuality,
    ...
    buildRow('home', {
      ...
      actionOn: enabled.home !== false && homeDisplay.sessionOpen === true,
      hintHe: enabled.home === false
        ? DISABLED_HE
        : (companion?.hint_he || COMM_LINK_META.home.hintHe),
      quality: homeQuality(companion, homeRttMs),
      active: false,
    }),
    buildRow('rc', {
