 ...
 *   altitudeSource: 'vfr_hud'|'global_relative'|'global_msl'|null,
 *   heading: number|null,
...
export function composeHudTelemetryFields(mavConn) {
  ...
  const takeAlt = (value, source) => {
    const n = sseFiniteNumber(value);
  ...
  if (vfrUsable) {
    takeAlt(vfr?.alt, 'vfr_hud')
      || (gpiUsable && takeAlt(gpi?.relativeAltM, 'global_relative'))
      || (gpiUsable && takeAlt(gpi?.altMslM, 'global_msl'));
  } else if (gpiUsable) {
    takeAlt(gpi?.relativeAltM, 'global_relative') || takeAlt(gpi?.altMslM, 'global_msl');
  } else {
    takeAlt(vfr?.alt, 'vfr_hud')
      || takeAlt(gpi?.relativeAltM, 'global_relative')
      || takeAlt(gpi?.altMslM, 'global_msl');
  }
  ...
  const climbRate = sseFiniteNumber(vfr?.climb);
