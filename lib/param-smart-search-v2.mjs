...
function toResultRow(base, score, reasoning, source) {
  const onFc = base.available_on_fc === true && base.live_value != null && base.live_value !== '';
  const geminiConfidence = source === 'gemini-rerank' && Number.isFinite(Number(reasoning?.confidence))
    ? Math.max(0, Math.min(1, Number(reasoning.confidence)))
    : null;
  return {
    ...
    available_on_fc: onFc,
    live_value: onFc ? base.live_value : null,
...
export async function runParamSmartSearchV2(q, { liveParams = null, maxResults = 5 } = {}) {
  ...
  if (autotuneIntent) {
    ...
    if (forced.length) {
      const rest = scored
        .filter((s) => !/^AUTOTUNE_/.test(s.row.param_key))
        .map((s) => toResultRow(s.row, s.score, heuristicReasons(query, s.row), 'deterministic'))
        .slice(0, Math.max(0, maxResults - forced.length));
      const topForced = [...forced, ...rest].slice(0, maxResults);
  ...
  if (reranked && reranked.length) {
    ...
    ranked = scored.map((s) => toResultRow(s.row, s.score, heuristicReasons(query, s.row), source));
  }
