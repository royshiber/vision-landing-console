...
/** SET_MODE (#11) — ArduPlane custom_mode only. Never ARM / DISARM. */
const MSG_SET_MODE = 11;
const MAV_MODE_FLAG_CUSTOM_MODE_ENABLED = 1;
const ASK_PLANE_MODE_LAND = 14;
...
/**
 * SET_MODE (#11) — ArduPilot custom_mode only.
...
export function buildSetModePayload(sysId, customMode) {
  ...
  const payload = Buffer.alloc(6);
  payload[0] = sysId & 0xff;
  payload[1] = MAV_MODE_FLAG_CUSTOM_MODE_ENABLED;
  payload.writeUInt32LE(mode >>> 0, 2);
  return payload;
}
...
export class MavlinkConnection extends EventEmitter {
  ...
  /**
   * Ask voice session: switch ArduPlane custom_mode (LAND / RTL / named mode).
   * SET_MODE only. Never ARM / DISARM. Never COMMAND_LONG 400.
   */
