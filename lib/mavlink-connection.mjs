...
export function buildMavlink2Frame(msgId24, payload, seq = 0) {
  const len = payload.length;
  const incompat = 0;
  const compat = 0;
...
export function parseSysStatus(p) {
  ...
  const loadRaw = p.readUInt16LE(12);
...
export class MavlinkConnection extends EventEmitter {
  ...
  getStatus() {
    ...
    return {
      ...
      rcAgeMs: this.lastRcAt != null ? now - this.lastRcAt : null,
      ...
      gpsFixType: Number.isFinite(this.lastGpsRaw?.fixType) ? this.lastGpsRaw.fixType : null,
      gpsSats: Number.isFinite(this.lastGpsRaw?.satellites) ? this.lastGpsRaw.satellites : null,
      ignoredHeartbeats: {
...
export function pickLiveMavlinkConnection(connections, commandLinkId = null) {
  ...
  const radioUp = connected.find((c) => c.linkRole !== 'cellular');
  if (radioUp) return radioUp;
