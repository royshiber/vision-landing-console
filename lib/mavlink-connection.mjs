...
export function hudMessageRateTargets() {
  return Object.freeze({
    streams: [
      { id: MAV_DATA_STREAM_POSITION, name: 'POSITION' },
      { id: MAV_DATA_STREAM_EXTRA1, name: 'EXTRA1' },
      { id: MAV_DATA_STREAM_EXTRA2, name: 'EXTRA2' },
    ],
    messages: [
      { id: MSG_ATTITUDE, name: 'ATTITUDE' },
      { id: MSG_GLOBAL_POSITION_INT, name: 'GLOBAL_POSITION_INT' },
      { id: MSG_VFR_HUD, name: 'VFR_HUD' },
    ],
  });
}
...
export class MavlinkConnection extends EventEmitter {
  ...
  /**
   * Ask the FC for HUD streams (ATTITUDE / VFR_HUD / GLOBAL_POSITION_INT).
   * Safe telemetry subscribe for TCP relay and other links — not a flight command.
   */
  requestHudMessageRates() {
    ...
    for (const stream of streams) {
      const payload = buildRequestDataStreamPayload(target, tc, stream.id, HUD_STREAM_RATE_HZ);
      const frame = this._preferredTxVersion === 2
        ? buildMavlink2Frame(MSG_REQUEST_DATA_STREAM, payload, this._seq++)
        : buildMavlink1Frame(MSG_REQUEST_DATA_STREAM, payload, this._seq++);
      this._send(frame);
    }
    for (const msg of messages) {
      const payload = buildSetMessageIntervalPayload(target, tc, msg.id, HUD_STREAM_INTERVAL_US);
      const frame = this._preferredTxVersion === 2
