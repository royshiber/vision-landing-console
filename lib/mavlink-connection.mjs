...
/** MAV_PARAM_TYPE values — we request float; FC will echo with its real type. */
...
export class MavlinkConnection extends EventEmitter {
   ...
   */
  setParam(paramName, value, { timeoutMs = 3000 } = {}) {
    return new Promise((resolve, reject) => {
      if (!this.connected) { reject(new Error('FC not connected')); return; }
      ...
      valBuf.writeFloatLE(value, 0);
      const tc = this.fcTargetCompId ?? 1;
      const payload = Buffer.concat([
        nameBuf,
        valBuf,
        Buffer.from([MAV_PARAM_TYPE_REAL32, target & 0xff, tc & 0xff]),
      ]);
      ...
      const timer = setTimeout(() => {
        this._pendingParamSets.delete(paramName);
        reject(new Error(`PARAM_SET timeout for ${paramName}`));
      }, timeoutMs);
      this._pendingParamSets.set(paramName, { resolve, reject, timer, expectedValue: value });
      this._send(frame);
    });
  }
    ...
    for (const frame of frames) {
      ...
      if (frame.msgId === MSG_HEARTBEAT) {
        ...
        if (pv) {
          ...
          this.emit('param', pv);
          // Resolve any pending setParam() waiting for echo of this param.
          const pending = this._pendingParamSets.get(pv.name);
          if (pending) {
            clearTimeout(pending.timer);
            this._pendingParamSets.delete(pv.name);
