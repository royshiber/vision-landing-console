...
export class MavlinkConnection extends EventEmitter {
  constructor({ id, name, type, host, port, serialPort, baudRate, linkRole }) {
    ...
    /** First accepted autopilot system id. Later heartbeats from other ids are ignored. */
  ...
  _acceptVehicleHeartbeat(frame, hb) {
    if (!hb || !frame) return false;
    if (hb.autopilot === MAV_AUTOPILOT_INVALID) return false;
    if (hb.type === MAV_TYPE_GCS) return false;
    if (frame.sysId === GCS_SYS_ID) return false;
    if (frame.compId !== MAV_COMP_ID_AUTOPILOT1) return false;
    if (this._lockedAutopilotSysId == null) {
      this._lockedAutopilotSysId = frame.sysId;
      return true;
    }
    return frame.sysId === this._lockedAutopilotSysId;
  }
  ...
  _handleData(data) {
    ...
    for (const frame of frames) {
      ...
      if (frame.msgId === MSG_HEARTBEAT) {
        ...
        if (ack) this.emit('command-ack', ack);
