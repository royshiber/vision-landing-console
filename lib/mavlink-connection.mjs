...
/**
 * Command/telemetry connection.
 * When two links are live, `_commandLinkId` (operator active-link pick) wins.
 * Falls back to the first live instance so existing callers keep working.
 */
export function getActiveConnection() {
  if (_commandLinkId != null) {
    const preferred = _active.get(_commandLinkId) ?? _active.get(Number(_commandLinkId));
    if (preferred) return preferred;
  }
  return _active.size > 0 ? [..._active.values()][0] : null;
}
