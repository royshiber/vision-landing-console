...
export function buildRetrievalContext(db, question, { limitNotes = 6, limitLogs = 6, flightId = null } = {}) {
  ...
  try {
    ...
    logger.error({ err }, 'buildRetrievalContext: DB query failed');
    return { block: '(שגיאת מסד נתונים בעת אחזור הקשר.)', meta: { notes: 0, logs: 0 } };
  }
...
export function getLatestCodeDigest(db) {
  try {
    ...
    logger.error({ err }, 'getLatestCodeDigest: DB query failed');
    return null;
  }
}
