...
export const DEV_TASK_TAXONOMIES = Object.freeze([
  'IDEA',
  'REQUEST',
  'IMPROVEMENT',
  'BUG',
...
const STORE_LOCK_TIMEOUT_MS = 4000;
const STORE_LOCK_STEP_MS = 20;
...
function withFileLock(filePath, fn, timeoutMs = STORE_LOCK_TIMEOUT_MS) {
  ensureDir(filePath);
  ...
  while (true) {
    try {
      ...
      if (Date.now() - started > timeoutMs) throw new Error('task store lock timeout');
      sleepMs(STORE_LOCK_STEP_MS);
    }
  }
  try {
    return fn();
...
function sanitizeTaskForStore(task) {
  ...
  return {
    ...
    taxonomy: normalizeEnum(task.taxonomy, DEV_TASK_TAXONOMIES, 'FEATURE') || 'FEATURE',
...
function readStore(filePath) {
  ...
  try {
    ...
    return tryRecoverFromBackup(filePath, 'malformed task store JSON');
  }
  if (!parsed || typeof parsed !== 'object' || !Array.isArray(parsed.tasks)) {
    return tryRecoverFromBackup(filePath, 'malformed task store schema');
  }
  const needsBackfill = parsed.tasks.some(taxonomyNeedsBackfill);
  const state = { tasks: parsed.tasks.map(sanitizeTaskForStore) };
  if (needsBackfill) writeStore(filePath, state);
  return state;
}

function tryRecoverFromBackup(filePath, reason) {
    ...
    try {
      ...
      if (backupRaw) {
        ...
        if (backup && typeof backup === 'object' && Array.isArray(backup.tasks)) {
          atomicWriteJson(filePath, backup);
          return { tasks: backup.tasks.map(sanitizeTaskForStore) };
        }
      }
    } catch {
...
function writeStore(filePath, state) {
  const nextPayload = { tasks: state.tasks.map(sanitizeTaskForStore) };
  const backupPath = `${filePath}.bak`;
  if (fs.existsSync(filePath)) {
    try {
      ...
      // current file corrupt or backup write failed — proceed with main write
    }
  }
  atomicWriteJson(filePath, nextPayload);
}
...
function buildTask(input) {
  ...
  const taxonomy = resolveCreateTaxonomy(input.taxonomy);
  ...
  pushAudit(task, 'created', null, { status, priority, taxonomy, target_area });
  return task;
}

function resolveCreateTaxonomy(value) {
  if (value == null || String(value).trim() === '') return 'FEATURE';
  const taxonomy = normalizeEnum(value, DEV_TASK_TAXONOMIES);
  if (!taxonomy) throw new Error('invalid taxonomy');
  return taxonomy;
}

function taxonomyNeedsBackfill(task) {
  if (!task || typeof task !== 'object') return true;
  return !DEV_TASK_TAXONOMIES.includes(String(task.taxonomy || '').trim().toUpperCase());
}
...
function applyFilters(tasks, { status, target, taxonomy, openOnly }) {
  ...
  const taxonomyNorm = taxonomy ? normalizeEnum(taxonomy, DEV_TASK_TAXONOMIES) : null;
  return tasks.filter((t) => {
    if (statusNorm && t.status !== statusNorm) return false;
    if (targetNorm && t.target_area !== targetNorm) return false;
    if (taxonomyNorm && t.taxonomy !== taxonomyNorm) return false;
...
export function createDevelopmentTaskStore(opts = {}) {
  ...
  function list(query = {}) {
    const state = withFileLock(filePath, () => readStore(filePath));
  ...
  function patch(id, updates = {}) {
    ...
    const allowed = new Set(['title', 'description', 'notes', 'priority', 'taxonomy', 'target_area', 'status']);
    ...
    if ('taxonomy' in updates) {
      const next = normalizeEnum(updates.taxonomy, DEV_TASK_TAXONOMIES);
      if (!next) throw new Error('invalid taxonomy');
      if (next !== task.taxonomy) {
        pushAudit(task, 'taxonomy_changed', { taxonomy: task.taxonomy }, { taxonomy: next });
        task.taxonomy = next;
      }
    }
