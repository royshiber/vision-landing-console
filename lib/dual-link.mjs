...
export function liveStatusToLinkState(live, { role = 'radio', modemPresent = true } = {}) {
  if (role === 'cellular' && !modemPresent && !live) return 'modem_absent';
...
export function hebrewLinkRole(role) {
  return role === 'cellular' ? 'סלולר' : 'טלמטריה רגילה';
}

export function hebrewPillLabel({ radio, cellular, active, bothConnected } = {}) {
  if (bothConnected) {
    return active === 'cellular' ? 'שני קישורים · סלולר' : 'שני קישורים · טלמטריה רגילה';
  }
  if (normalizeLinkState(cellular) === 'connected') return 'מחובר · סלולר';
  if (normalizeLinkState(radio) === 'connected') return 'מחובר · טלמטריה רגילה';
...
export function annotatedVideoAvailability({ cellular, modemPresent } = {}) {
  ...
  if (cell === 'connected') {
    return {
      ...
      reasonHe: 'ראייה מסומנת זמינה דרך סלולר ממחשב משימה.',
    };
  }
  if (!modemPresent || cell === 'modem_absent') {
    return {
      available: false,
      path: ANNOTATED_VIDEO_PATH,
      reason: 'modem_absent',
      reasonHe: 'אין שידור. מודם סלולר לא מחובר. ראייה מסומנת מגיעה רק ממחשב משימה.',
    };
  }
  return {
    available: false,
    ...
    reasonHe: 'אין שידור. סלולר מנותק. ראייה מסומנת לא עוברת ברדיו.',
  };
}
...
export function summarizeDualLink({
  ...
  const cellularState = normalizeLinkState(cellular);
  ...
  const video = annotatedVideoAvailability({ cellular: cellularState, modemPresent });
  return {
    radio: radioState,
    ...
    cellularLabelHe: `${hebrewLinkRole('cellular')} (Jetson)`,
