...
/**
 * Command/telemetry ops follow the operator-picked MAVLink link.
 * Companion-HTTP is never the flight command path. Video stays cellular-only.
 */
export function commandLinkOpsPath({
  active = null,
  commandLinkId = null,
  modemPresent = false,
  ...
  const link = active === 'cellular' || active === 'radio' ? active : null;
  return {
    commandLink: link,
    commandLinkId: commandLinkId == null || commandLinkId === '' ? null : Number(commandLinkId),
    paramsFollowCommandLink: true,
    paramsVia: link === 'cellular' ? 'cellular_mavlink' : link === 'radio' ? 'radio_mavlink' : null,
    companionHttpCommandPath: false,
    companionStatusReachable: companionReachable === true,
    jetsonReachableViaTailscaleOrCell: companionReachable === true,
    annotatedVideo: video && typeof video === 'object'
      ? {
          available: video.available === true,
          path: video.path || ANNOTATED_VIDEO_PATH,
          reason: video.reason || (modemPresent ? 'stream_absent' : 'modem_absent'),
          neverRadio: true,
        }
      : {
          available: false,
          path: ANNOTATED_VIDEO_PATH,
          reason: modemPresent ? 'stream_absent' : 'modem_absent',
          neverRadio: true,
        },
...
export const UPDATE_READINESS_HE = Object.freeze({
  jetsonWait: 'מחשב משימה לא מגיב. חברו רשת בית או סלולר.',
  jetsonOk: 'מחשב משימה מגיב.',
  companionWait: 'התקנת תוכנת מחשב משימה מחכה להגעה.',
  companionOk: 'מחשב משימה מגיע. התקנה ידנית מהקונסולה. בלי הפעלה אוטומטית.',
  fcWait: 'עדכון בקר נשאר לאישור. אין הבזקה מכאן.',
  versionsWait: 'גרסאות בקר ומחשב משימה לא מדווחות.',
  qualityWait: 'אין מד איכות. אין אחוז מזויף.',
  backupNote: 'גיבוי והחזרה על המפעיל. אין מסלול אוטומטי.',
});

/**
 * Thin update-over-Jetson readiness. No FC flash button. No Companion apply/restart.
 */
export function cellularUpdateReadiness({
  companionReachable = false,
} = {}) {
  const jetsonOk = companionReachable === true;
  return {
    autoDeployFc: false,
    fcFirmwareHumanGate: true,
    companionApplyRestart: false,
    existingInstallPath: '/api/jetson/install',
    steps: [
      {
        id: 'jetson_reachable',
        ok: jetsonOk,
        later: false,
        humanGate: false,
        he: jetsonOk ? UPDATE_READINESS_HE.jetsonOk : UPDATE_READINESS_HE.jetsonWait,
      },
      {
        ...
        later: !jetsonOk,
        humanGate: false,
        he: jetsonOk ? UPDATE_READINESS_HE.companionOk : UPDATE_READINESS_HE.companionWait,
      },
      {
        id: 'fc_firmware',
        ok: false,
        later: true,
        humanGate: true,
        he: UPDATE_READINESS_HE.fcWait,
      },
    ],
  };
}

const ANNOTATED_STREAM_URL_RE = /^(rtsp|rtsps|srt|udp|http|https):\/\//i;

/** Real annotated stream URL/path. Placeholders and empty values never count. */
export function isRealAnnotatedStreamUrl(value) {
  const url = String(value || '').trim();
  if (!url) return false;
  const lower = url.toLowerCase();
  if (lower === 'none' || lower === 'off' || lower === 'stopped' || lower === 'idle'
    || lower === 'absent' || lower === 'null' || lower === 'false') {
    return false;
  }
  return ANNOTATED_STREAM_URL_RE.test(url);
}

/**
 * Companion annotated stream is present only with a real named pipe + fps,
 * or an explicit stream URL. Radio never supplies this.
 */
export function annotatedStreamPresent(companion) {
  ...
  const health = companion.health && typeof companion.health === 'object'
    ? companion.health
    : {};
  const video = overlay.video || health.video || companion.video;
  if (!video || typeof video !== 'object') return false;
  if (isRealAnnotatedStreamUrl(
    video.annotated_stream_url || video.annotatedStreamUrl || video.streamUrl,
  )) {
    return true;
  }
  const pipe = String(video.annotated_pipeline || video.annotatedPipeline || 'none')
    .trim()
    .toLowerCase();
  if (!pipe || pipe === 'none' || pipe === 'off' || pipe === 'stopped' || pipe === 'idle') {
    return false;
  }
  const fps = Number(video.annotated_fps ?? video.annotatedFps);
