...
import { ASK_PLANE_MODE_LAND, ASK_PLANE_MODE_RTL } from './mavlink-connection.mjs';
import { isAskSessionFlightOpKind } from './assist/ask-safety.mjs';
import { logger } from './logger.mjs';
import { getCorrelationId } from './request-context.mjs';

const PLANE_MODE_BY_NAME = Object.freeze(Object.fromEntries(
...
export function resolveAskFlightOpCustomMode(kind, slots = {}) {
  ...
  if (k === 'ARM' || k === 'DISARM') return null;
  if (k === 'LAND') return ASK_PLANE_MODE_LAND;
  if (k === 'RTL') return ASK_PLANE_MODE_RTL;
  if (k === 'MODE_CHANGE') {
    const name = String(slots.mode || '').trim().toUpperCase();
    if (!name) return null;
    if (name in PLANE_MODE_BY_NAME) return PLANE_MODE_BY_NAME[name];
    const asNum = Number(name);
    if (Number.isInteger(asNum) && ARDUPILOT_PLANE_MODES[asNum]) return asNum;
  }
  return null;
}

/**
 * Ask voice session flight op — LAND / RTL / mode via SET_MODE.
...
export async function applyAskFlightOp(
  ...
  const customMode = resolveAskFlightOpCustomMode(k, { mode });
  if (customMode == null) {
    return {
      ok: false,
      sent: false,
