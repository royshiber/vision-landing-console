...
/** Voice SET_MODE names. Anything else is refused. AVOID_ADSB and INITIALISING are never selectable. */
const PLANE_VOICE_MODES = new Set([
  'MANUAL', 'STABILIZE', 'FBWA', 'FBWB', 'CRUISE', 'AUTOTUNE', 'AUTO',
  'RTL', 'LOITER', 'CIRCLE', 'GUIDED', 'TAKEOFF',
  'QSTABILIZE', 'QHOVER', 'QLOITER', 'QRTL',
]);

/** These names are landing, not SET_MODE. DO_LAND_START only when the mission has the marker. */
const PLANE_LAND_MODES = new Set(['LAND', 'AUTOLAND', 'QLAND', 'LOITER_ALT_QLAND']);

/**
 * Copter voice SET_MODE names. Blocked: ACRO, FLIP, THROW, TURTLE, AUTOROTATE,
 * SYSTEMID, AVOID_ADSB, and LAND (fixed-wing landing command only).
 */
const COPTER_VOICE_MODES = new Set([
  'STABILIZE', 'ALT_HOLD', 'AUTO', 'GUIDED', 'LOITER', 'RTL', 'CIRCLE',
  'DRIFT', 'SPORT', 'AUTOTUNE', 'POSHOLD', 'BRAKE', 'GUIDED_NOGPS',
  'SMART_RTL', 'FLOWHOLD', 'FOLLOW', 'ZIGZAG', 'AUTO_RTL',
]);

export function isNumericAskMode(raw) {
  return /^\d+$/.test(String(raw || '').trim());
}

export function isAskVoiceLandMode(raw, family = 'plane') {
  const name = String(raw || '').trim().toUpperCase();
  if (!name || isNumericAskMode(name)) return false;
  if (family === 'copter') return name === 'LAND';
  return PLANE_LAND_MODES.has(name);
}

function voiceModeAllowed(family, name) {
  const allow = family === 'copter' ? COPTER_VOICE_MODES : PLANE_VOICE_MODES;
...
export function resolveAskFlightOpCustomMode(kind, slots = {}) {
  ...
  if (k === 'RTL') {
    if (!voiceModeAllowed(family, 'RTL')) return null;
    return family === 'copter' ? MODE_BY_NAME.copter.RTL : ASK_PLANE_MODE_RTL;
  }
  if (k === 'MODE_CHANGE') {
    const name = String(slots.mode || '').trim().toUpperCase();
    if (!name || isNumericAskMode(name) || isAskVoiceLandMode(name, family)) return null;
    if (!voiceModeAllowed(family, name)) return null;
...
export async function applyAskFlightOp(
  ...
  if (k === 'MODE_CHANGE' && isNumericAskMode(requestedName)) {
    return {
      ok: false,
      sent: false,
      error: 'numeric_mode',
      customMode: null,
      note: ASSIST_HE.flightOpNumericMode,
    };
  }
  if (k === 'LAND' || isAskVoiceLandMode(requestedName, family)) {
  ...
  if (customMode == null) {
    const named = k === 'MODE_CHANGE' && requestedName;
    return {
      ok: false,
      sent: false,
      error: named ? 'mode_not_allowed' : 'unknown_mode',
      customMode: null,
      note: named ? ASSIST_HE.flightOpModeNotAllowed : ASSIST_HE.flightOpUnknownMode,
    };
  }
...
async function applyAskDoLandStart(
  ...
  let items = [];
  try {
    await mavConn.refreshMissionToCache({ timeoutMs: missionTimeoutMs, exclusive: true });
