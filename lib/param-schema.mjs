import { coercePeripheralWriteValue } from './fc-peripheral-wizard.mjs';
import { coerceGroupWriteValue } from './fc-param-groups.mjs';

const COMPANION_PORT_MIN = 1;
const COMPANION_PORT_MAX = 8;
const COMPANION_DEFAULT_PORT = 4;
const COMPANION_DEFAULT_SR = 4;
...
export function coerceArduTargetPatch(rawTarget = {}, defaults = buildArduTargetDefaults()) {
  ...
  for (const [key, rawValue] of Object.entries(rawTarget)) {
    const peripheralWrite = coercePeripheralWriteValue(key, rawValue);
    if (peripheralWrite && !(key in defaults)) {
      if (!peripheralWrite.ok) {
        rejected[key] = peripheralWrite.reason;
        continue;
      }
      accepted[key] = peripheralWrite.value;
      continue;
    }

    const groupWrite = coerceGroupWriteValue(key, rawValue);
    if (groupWrite && !(key in defaults)) {
      if (!groupWrite.ok) {
        rejected[key] = groupWrite.reason;
        continue;
      }
      accepted[key] = groupWrite.value;
      continue;
    }
    ...
    if (serialProtocolMatch) {
      if (!peripheralWrite || !peripheralWrite.ok) {
        rejected[key] = peripheralWrite?.reason || 'invalid_enum';
        continue;
      }
      accepted[key] = peripheralWrite.value;
      continue;
