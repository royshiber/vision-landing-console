...
async function applyFcParamChange(db, action, row, ctx) {
  ...
  let verified = false;
  let error = null;
  const timeoutMs = /^(SERIAL\d|SR\d|EK3_|GPS_|COMPASS_|FLOW_|CAM)/i.test(param) ? 9000 : 6000;
  try {
    const result = await mavConn.setParam(param, to, { timeoutMs, retries: 1 });
  ...
  if (verified) {
    markApplied(db, row.id);
    logger.info({ corrId: getCorrelationId(), actionId: row.id, param, from, to }, 'fc param applied');
    return {
      ok: true,
      kind: 'param_change',
      target: 'fc',
  ...
  logger.warn({ corrId: getCorrelationId(), actionId: row.id, param, from, to, error }, 'fc param apply failed; rollback attempted');
  const hint =
    /timeout/i.test(error || '') && /^SERIAL/i.test(param)
      ? `${error} — ייתכן שצריך reboot לבקר אחרי SERIAL; ודא חיבור MAVLink יציב ו-DISARMED`
      : error;
  throw new ApplyError(500, 'apply_failed', `FC write/verify failed: ${hint}`);
}
