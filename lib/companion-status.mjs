...
import { normalizeOpticalNav } from './optical-nav.mjs';
...
function ramPct(system) {
  const direct = finiteOrNull(system.memPct ?? system.mem_pct ?? system.ram_pct);
...
function systemCpuPct(system) {
  return finiteOrNull(system.cpu_percent ?? system.cpuLoadPct ?? system.cpu_load_pct ?? system.cpu);
}

function systemTempC(system) {
  return finiteOrNull(system.temperature_c ?? system.tempC ?? system.temp_c ?? system.temp);
}

function sysStatusMetric(fc, keys) {
  const fields = obj(obj(fc.sys_status)?.fields) || obj(fc.sys_status) || {};
  const pools = [fields, obj(fc) || {}];
  for (const pool of pools) {
    for (const key of keys) {
      let n = finiteOrNull(pool[key]);
      if (n == null) continue;
      if ((key === "load" || key === "cpu_load") && n > 100 && n <= 1000) n = n / 10;
      return n;
    }
  }
  return null;
...
export function mapCompanionStatus(raw, opts = {}) {
  ...
  const opticalNav = normalizeOpticalNav(
    src.optical_nav || src.opticalNav || obj(src.extras)?.optical_nav || obj(src.extras)?.opticalNav,
  );
  ...
  const mapped = {
    ...
    system: {
      ...
      cpu_percent: systemCpuPct(system),
      ...
      temperature_c: systemTempC(system),
      disk_used_percent: finiteOrNull(system.disk_used_percent),
      cpuLoadPct: systemCpuPct(system),
      tempC: systemTempC(system),
    ...
    fc: {
      ...
      loadPct: sysStatusMetric(fc, ["load_pct", "cpu_load", "load"]),
      memPct: sysStatusMetric(fc, ["mem_pct", "ram_pct", "memory_pct"]),
      tempC: sysStatusMetric(fc, ["temperature_c", "temp_c", "temperature"]),
    ...
    vision: {
      ...
      health: visionHealth,
      fps: measuredOrNull(vision.fps, vision.running === true || boolOrNull(vision.camera_ok) === true),
      latency_ms: measuredOrNull(visionResult.latency_ms ?? vision.latency_ms, visionLive),
      camera_ok: boolOrNull(vision.camera_ok),
      last_frame_age_ms: finiteOrNull(vision.last_frame_age_ms ?? vision.lastFrameAgeMs ?? vision.age_ms),
      frame_count: finiteOrNull(vision.frame_count ?? vision.frameCount),
      cameras: obj(vision.cameras) || obj(opticalNav.cameras) || null,
      dry_run: boolOrNull(vision.dry_run ?? vision.dryRun) === true,
      real: boolOrNull(vision.real) === true,
      ingest_source: vision.source || null,
    ...
    landing: {
      ...
      runway_detector: boolOrNull(landing.runway_detector ?? landing.runwayDetector ?? obj(landing.runway)?.present),
      runway_detected: boolOrNull(landing.runway_detected ?? landing.runwayDetected),
      lock_state: landing.lock_state ?? landing.lockState ?? null,
      runway: obj(landing.runway) || null,
    },
    video: {
      status: states.video,
      ...
      annotated_stream_url: video.annotated_stream_url || video.annotatedStreamUrl || video.streamUrl || null,
      neverRadio: video.neverRadio !== false,
      radioSatisfies: false,
      annotatedReason: video.reason || video.annotated_reason || null,
    ...
    diagnostics,
    extras: obj(src.extras) || null,
    modem: obj(src.modem) || obj(obj(src.health)?.modem) || null,
  ...
  mapped.compatibility = {
    ...
    vision: {
      ...
      lastFrameAgeMs: mapped.vision.last_frame_age_ms,
      cameras: mapped.vision.cameras,
      dryRun: mapped.vision.dry_run === true,
      real: mapped.vision.real === true,
      navLat: opticalNav.lat,
      navLon: opticalNav.lon,
      opticalNav,
    },
    slam: {
