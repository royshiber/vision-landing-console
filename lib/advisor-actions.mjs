...
/** @param {string} param */
export function getParamDiscreteMeta(param) {
  const info = getParamInfo(param);
  if (!info) return null;
  if (info.values && typeof info.values === 'object' && Object.keys(info.values).length > 0) {
    return { kind: 'enum', values: info.values };
  }
  if (info.bitmask && typeof info.bitmask === 'object' && Object.keys(info.bitmask).length > 0) {
    return { kind: 'bitmask', values: info.bitmask };
  }
  return null;
}

export function isDiscreteArduParam(param) {
  return getParamDiscreteMeta(param) != null;
}

/** @returns {number[]} */
function enumKeys(meta) {
  return Object.keys(meta.values)
    .map((k) => Number(k))
    .filter((n) => Number.isFinite(n))
    .sort((a, b) => a - b);
}

/** Snap to nearest valid enum/bitmask integer key. */
export function snapDiscreteParamValue(param, value) {
  const meta = getParamDiscreteMeta(param);
  if (!meta || !isFiniteNumber(Number(value))) return Number(value);
  const keys = enumKeys(meta);
  if (!keys.length) return Math.round(Number(value));
...
export function isValidDiscreteParamValue(param, value) {
  const meta = getParamDiscreteMeta(param);
  if (!meta) return true;
  const v = Math.round(Number(value));
  return enumKeys(meta).includes(v);
}

/** Human label e.g. "MAVLink2 (2)" for UI and advisor cards. */
export function formatDiscreteParamLabel(param, value) {
  const meta = getParamDiscreteMeta(param);
  if (!meta) return null;
  const v = Math.round(Number(value));
  const label = meta.values[String(v)] ?? meta.values[v];
  if (!label) return `${param} = ${v}`;
  return `${label} (${v})`;
}
...
export function buildParamAlternatives(p) {
  const { from, primaryTo, spec, param } = p;
  const toN = roundParamValueForSpec(spec, primaryTo);

  if (param && isDiscreteArduParam(param)) {
    const snapped = snapDiscreteParamValue(param, toN);
    if (!isValidDiscreteParamValue(param, snapped)) return [];
    const enumLabel = formatDiscreteParamLabel(param, snapped);
    return [{ id: 'primary', label: enumLabel || 'לפי ההמלצה', to: snapped, isPrimary: true, enumLabel }];
  }
...
export function isValueInParamProposalFamily(param, from, primaryTo, value) {
  ...
  const alts = buildParamAlternatives({ from: from != null ? Number(from) : null, primaryTo, spec: resolved.spec, param });
...
export function enrichParamChangeOption(option) {
  ...
  if (isDiscreteArduParam(ch.param)) {
    withHelp.discreteKind = getParamDiscreteMeta(ch.param)?.kind || 'enum';
    withHelp.enumLabel = formatDiscreteParamLabel(ch.param, ch.to);
  }
  return withHelp;
}
...
function validateParamChange(raw) {
  ...
  if (isDiscreteArduParam(param)) {
    if (Math.abs(to - Math.round(to)) > 1e-3) {
      return {
        ok: false,
        reason: `param_change: ${param} is enum/bitmask — use a whole valid option (not ${to})`,
      };
    }
    to = snapDiscreteParamValue(param, to);
    if (!isValidDiscreteParamValue(param, to)) {
      return { ok: false, reason: `param_change: ${param} value ${to} is not a valid enum option` };
    }
  }
...
- **פרמטרי enum/bitmask** (למשל SERIALn_PROTOCOL, GPS_TYPE, EK3_SRC1_POSXY): השתמש **רק** בערכים שלמים מהרשימה הרשמית (1=MAVLink1, 2=MAVLink2, 5=GPS…). **אסור** ערכים שבריים או "שמרני/נועז" — הצע ערך enum אחד מדויק בלבד.
