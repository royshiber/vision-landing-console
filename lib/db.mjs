...
export function openDatabase(dbFilePath) {
  ...
  db.exec(`
    ...
    /** Why: persist operator connection profiles for GET/POST/PATCH/DELETE /api/connections.
     *  What: columns match SQL already issued by lib/routes/core-api.mjs and lib/smart-connect.mjs.
     *  Additive: old DBs gain the table on next openDatabase(). */
    CREATE TABLE IF NOT EXISTS connections (
      ...
      last_connected TEXT,
      link_role      TEXT    NOT NULL DEFAULT 'radio'
    );

    /** Why: dedicated MAVLink/flight telemetry archive under data/flights/archive.
     *  What: one row per operator-armed session file; downlink_state stays local until the modem path is live.
     *        Process boot marks leftover open rows interrupted and sets ended_at. */
    CREATE TABLE IF NOT EXISTS telemetry_archive (
      ...
      ended_at        TEXT,
      downlink_state  TEXT    NOT NULL DEFAULT 'local'
    );
    CREATE INDEX IF NOT EXISTS idx_telemetry_archive_started ON telemetry_archive(started_at DESC);
  `);

  try {
    const cols = db.prepare(`PRAGMA table_info(connections)`).all();
    if (!cols.some((c) => c && c.name === 'link_role')) {
      db.exec(`ALTER TABLE connections ADD COLUMN link_role TEXT NOT NULL DEFAULT 'radio'`);
    }
  } catch {
    /* existing DBs without the column still work via COALESCE in queries */
  }

  migrateFlightBook(db);

  return db;
}

/** Why: cloud flight book is additive. Old DBs keep manual flights and gain nullable columns plus event/artifact tables. */
function migrateFlightBook(db) {
  const cols = db.prepare(`PRAGMA table_info(flights)`).all();
  const have = new Set(cols.map((c) => c && c.name).filter(Boolean));
  const add = [
    ['flight_uid', 'TEXT'],
    ...
    ['air_time_s', 'REAL'],
    ['max_rel_alt_m', 'REAL'],
    ['max_airspeed_mps', 'REAL'],
    ['max_groundspeed_mps', 'REAL'],
    ['distance_m', 'REAL'],
    ['modes_json', 'TEXT'],
    ['warnings', 'INTEGER'],
    ['errors', 'INTEGER'],
  ...
  for (const [name, ddl] of add) {
    if (!have.has(name)) db.exec(`ALTER TABLE flights ADD COLUMN ${name} ${ddl}`);
  }
  db.exec(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_flights_flight_uid
      ON flights(flight_uid) WHERE flight_uid IS NOT NULL;

    CREATE TABLE IF NOT EXISTS flight_cloud_artifacts (
    ...
    CREATE TABLE IF NOT EXISTS flight_events (
      ...
      data_json TEXT,
      cause_id TEXT,
      PRIMARY KEY (flight_uid, event_id)
    );
    CREATE INDEX IF NOT EXISTS idx_flight_events_type ON flight_events(type);
    CREATE INDEX IF NOT EXISTS idx_flight_events_sev ON flight_events(sev);
    ...
    CREATE INDEX IF NOT EXISTS idx_flight_debriefs_uid ON flight_debriefs(flight_uid, created_at DESC);
  `);
}
