...
function answerForQuestion(intentResult, ctx) {
  ...
  if (topic === 'evolve') {
    ...
    if (t) return `Active development task: ${t.title || t.id} (${t.status || 'unknown'}). Agent is not auto-started by Assist.`;
    return 'No active development task is present in Assist context. Open Evolve to manage tasks.';
  }
...
export function createAssistService({
  ...
  async function processInput({ text, channel = 'text', context_snapshot = {} }) {
    ...
    if (resolved.intent === 'DEVELOPMENT' || resolved.intent === 'REQUEST') {
      ...
      const resp = buildResponse({
        answer: 'I can create a Development Task for that. Create the task?',
        intent: resolved.intent,
        confidence: resolved.confidence,
        context,
        action_proposal: proposal,
        requires_confirmation: true,
        next_step: 'Confirm to create a DRAFT development task. Agent will not start automatically.',
        kind: 'ACTION_REQUIRING_CONFIRMATION',
      });
      session.messages.push({ role: 'assist', ...resp });
      return resp;
  ...
  async function confirmProposal({ proposal_id, confirm }) {
    ...
    if (!proposal) {
      return {
        ...
        answer: 'That proposal is no longer available.',
      };
    }
    if (!confirm) {
    ...
    if (action === 'CREATE_NOTE') {
      ...
      // Never start agent / release / deploy from Assist
      result = {
        task: {
          id: created.id,
          title: created.title,
          status: created.status,
          agent_state: created.agent?.state || 'NOT_STARTED',
        },
        navigation: proposal.payload.navigate_after
    ...
    return {
      ...
      answer: action === 'CREATE_DEVELOPMENT_TASK'
        ? `Development Task created (${result.task.id}). Opening Evolve. Agent was not started.`
        : action === 'CREATE_NOTE'
          ? 'Note saved.'
