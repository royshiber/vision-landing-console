...
export function createAssistService({
  ...
  function makeProposal(actionType, payload, { requiresConfirmation = true } = {}) {
    ...
    // Roy lock ask_voice_flight_confirm_always — flight-affecting never auto-applies.
    const mustConfirm = askRequiresConfirmation(actionType, requiresConfirmation);
    purgeExpired();
    const id = randomUUID();
    const proposal = {
      id,
  ...
  function buildResponse({
    ...
    return {
      ...
      suggestion,
      ask_voice_flight_confirm_always: ASK_VOICE_FLIGHT_CONFIRM_ALWAYS,
  ...
  function attachSuggestion(resp, context, text) {
    ...
    if (suggestion.writes_fc) suggestion.requires_confirmation = true;
  ...
  async function processInput({ text, channel = 'text', context_snapshot = {} }) {
    ...
    if (resolved.intent === 'FLIGHT_PARAM' && resolved.slots?.key) {
      const proposal = makeProposal('PROPOSE_PARAM_CHANGE', {
  ...
  async function confirmProposal({ proposal_id, confirm, start_agent }) {
    ...
    if (action === 'CREATE_NOTE') {
      ...
      const key = String(proposal.payload?.key || '').trim().toUpperCase();
      const value = Number(proposal.payload?.value);
      ...
      if (typeof applyParamChange !== 'function') {
        ...
        try {
          ...
          result = {
            ...
            note: applied?.note || (offline ? ASSIST_HE.paramAppliedOffline : ASSIST_HE.paramApplied),
