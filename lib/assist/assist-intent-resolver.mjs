...
import { resolveAskFlightIntent } from './ask-flight-intents.mjs';
...
/**
 * Legacy English patterns kept as a second pass. Flight ARM/LAND/param
 * intents are resolved first by resolveAskFlightIntent (Roy 2026-09-12):
 * param set/change becomes PROPOSE_PARAM_CHANGE, not PARAM_WRITE.
 */
...
function isAdvisorStyleQuery(q) {
  if (hasAnyToken(q, HE_DEV_VERBS) && hasProductChangeObject(q)) return false;
  if (hasToken(q, 'יועץ') || /\badvisor\b/i.test(q)) return true;
  if (hasAnyToken(q, ['נדנוד', 'הצפה'])) return true;
  if (hasToken(q, 'נחיתה קשה')) return true;
  if (hasToken(q, 'מה לשנות') || hasToken(q, 'איזה פרמטר')) return true;
  if (/מה השינויים האחרונים/.test(q)) return true;
  if (/\babort\b/i.test(q) && hasAnyToken(q, ['לכוונן', 'שגוי', 'שגויים'])) return true;
  return false;
}

function isFlightEngineerStyleQuery(q) {
  if (hasAnyToken(q, HE_DEV_VERBS) && hasProductChangeObject(q)) return false;
  if (isAdvisorStyleQuery(q)) return false;
  if (hasAnyToken(q, ['מהנדס', 'מהנדס טיסה', 'מהנדס קולי'])) return true;
  if (/\bflight engineer\b/i.test(q) || /\bvoice engineer\b/i.test(q)) return true;
  if (hasToken(q, 'פתק קולי') || hasToken(q, 'הערת קול')) return true;
  if (/\bops notes?\b/i.test(q) || /\bvoice notes?\b/i.test(q)) return true;
  return false;
}

function isArduLabStyleQuery(q) {
  if (hasAnyToken(q, HE_DEV_VERBS) && hasProductChangeObject(q)) return false;
  if (isAdvisorStyleQuery(q) || isFlightEngineerStyleQuery(q)) return false;
  if (hasToken(q, 'ארדולאב') || hasToken(q, 'ardulab')) return true;
  if (/\bardulab\b/i.test(q) || /\bfeature designer\b/i.test(q)) return true;
  if (hasToken(q, 'מעצב פיצ׳רים') || hasToken(q, "מעצב פיצ'רים")) return true;
  if (hasToken(q, 'פיצ׳ר מותאם') || hasToken(q, "פיצ'ר מותאם")) return true;
  return false;
}

function isAutoConfigStyleQuery(q) {
  if (hasAnyToken(q, HE_DEV_VERBS) && hasProductChangeObject(q)) return false;
  if (isAdvisorStyleQuery(q) || isFlightEngineerStyleQuery(q) || isArduLabStyleQuery(q)) return false;
  if (hasToken(q, 'אשף')) return true;
  if (hasToken(q, 'קונפיג אוטומטי') || hasToken(q, 'קונפיגורציה אוטומטית')) return true;
  if (hasToken(q, 'אשף קונפיגורציה')) return true;
  if (/\bauto-?config\b/i.test(q) || /\bautoconfig\b/i.test(q)) return true;
  return false;
}
...
export function resolveAssistIntent(rawText, context = {}) {
  ...
  const flight = resolveAskFlightIntent(text);
  if (flight) {
    return flight;
  }
  ...
  if (
    ...
    const body = text
      ...
      .replace(/^כתוב הערה\s*(ש|על|:)?\s*/i, '')
      .replace(/^רשום הערה\s*(ש|על|:)?\s*/i, '')
      .replace(/^הערה\s*[:：]\s*/i, '')
      .trim() || text;
    return {
      intent: 'NOTE',
  ...
  if (isAdvisorStyleQuery(q)) {
    const route = findAssistRoute('advisor');
    if (route) {
      return {
        intent: 'UI_ACTION',
        confidence: 0.88,
  ...
  if (isFlightEngineerStyleQuery(q)) {
    const route = findAssistRoute('engineer');
    if (route) {
      return {
        intent: 'UI_ACTION',
        confidence: 0.88,
  ...
  if (isArduLabStyleQuery(q)) {
    const route = findAssistRoute('ardulab');
    if (route) {
      return {
        intent: 'UI_ACTION',
        confidence: 0.88,
  ...
  if (isAutoConfigStyleQuery(q)) {
    const route = findAssistRoute('auto-config');
    if (route) {
      return {
        intent: 'UI_ACTION',
        confidence: 0.88,
  ...
  const heDevTask = /משימ(?:ה|ת|ות)\s+פיתוח/.test(q);
  ...
  if (
    !hebrewQuestion
    && (
      hasToken(q, 'יכולת חדשה')
      || /\bnew capability\b/i.test(q)
      || /\badd a capability\b/i.test(q)
    )
  ) {
    return {
      intent: 'DEVELOPMENT',
...
function inferQuestionTopic(q, context) {
  if (/גובה|altitude|\balt\b|כמה גבוה|how high|height/.test(q)) return 'altitude';
  if (/קצב אנכי|vertical (?:speed|rate)|climb rate/.test(q)) return 'climb';
  if (/מרחק|distance/.test(q)) return 'distance';
  if (/מהירות|airspeed|groundspeed|\bspeed\b/.test(q)) return 'speed';
  if (/מוד|מצב טיסה|flight mode/.test(q)) return 'mode';
  if (/סוללה|battery|מתח/.test(q)) return 'battery';
  if (/קישור|link|rssi/.test(q)) return 'link';
  if (/gps/.test(q)) return 'gps';
  if (/vision|confidence/.test(q)) return 'vision';
  if (/looking at|what am i|מה אני רואה|מה מוצג/.test(q)) return 'seeing';
