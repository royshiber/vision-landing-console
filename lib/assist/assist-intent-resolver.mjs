...
function isAdvisorStyleQuery(q) {
  ...
  if (hasAnyToken(q, ['נדנוד', 'הצפה', 'יועץ מעבדה'])) return true;
...
function isFlightEngineerStyleQuery(q) {
  ...
  if (hasAnyToken(q, ['מהנדס', 'מהנדס טיסה', 'מהנדס מעבדה', 'מהנדס קולי'])) return true;
...
export function resolveAssistIntent(rawText, context = {}) {
  ...
  if (q === 'סימולציה' || q === 'sitl' || q === 'simulation') {
    const route = findAssistRoute('lab');
    if (route) {
      return {
        intent: 'UI_ACTION',
        confidence: 0.9,
