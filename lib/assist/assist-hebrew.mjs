...
export function hebrewReadbackAnswer(topic, ac) {
  if (topic === 'altitude') {
    return [ASSIST_HE.readoutAltitude, dashOrNumber(ac?.altitude_m), ac?.connected ? 'm' : ''].filter(Boolean).join('\n');
  }
  if (topic === 'speed') {
    const air = dashOrNumber(ac?.airspeed_ms);
    const gs = dashOrNumber(ac?.groundspeed_ms);
