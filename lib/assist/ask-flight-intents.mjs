...
const PARAM_PROPOSE_PATTERNS = [
  ...
  new RegExp(`\\b(?:can you|could you|please)\\s+(?:set|change|write)\\s+(?:param(?:eter)?\\s+)?${PARAM_KEY}\\s*(?:to|=)\\s*${PARAM_NUM}`, 'i'),
  new RegExp(`\\b${PARAM_KEY}\\s*=\\s*${PARAM_NUM}\\b`),
  new RegExp(`(?:שנה|קבע|הגדר|עדכן|תוריד|תחליף|תשנה|תעלה)\\s+(?:את\\s+)?${PARAM_KEY}\\s+(?:ל־|ל-|ל\\s+|=)\\s*${PARAM_NUM}`),
];
...
const HARD_BLOCKED_PATTERNS = [
  ...
  { re: /תחמש|תן(?:י)?(?: לי)? חימוש|תפעיל(?:י)?(?: לי)? את המנועים|הפעל מנועים|תדליק(?:י)?(?: לי)? (?:את )?המנועים/, kind: 'ARM' },
...
const READBACK_PATTERNS = [
  { re: /מה הגובה|גובה כמה|כמה גבוה|גובה נוכחי|what(?:'s| is) (?:the |our )?alt(?:itude)?|how high|what(?:'s| is) (?:our |the )?height|read (?:the )?alt/i, topic: 'altitude' },
  { re: /מה המהירות|מהירות כמה|what(?:'s| is) (?:the )?(?:air)?speed|groundspeed/i, topic: 'speed' },
  { re: /מה הקצב האנכי|קצב אנכי|vertical (?:speed|rate)|climb rate|rate of climb/i, topic: 'climb' },
  { re: /מה המרחק|כמה מרחק|what(?:'s| is) (?:the )?distance/i, topic: 'distance' },
...
function parseLandOrRtl(text) {
  ...
  if (
    /\b(land\s+now|auto[\s-]*land|autoland)\b/i.test(text)
    || /נחיתה אוטומטית|נחת(?:י|ה)?\s+עכשיו|נחיתה עכשיו|תנחית|תרד לנחיתה/.test(text)
    || /^(נחת|נחיתה)$/.test(text)
    || /\bland\b/i.test(text)
    || /\b(bring|put)\s+(it|her|us|the\s+plane)\s+down\b/i.test(text)
  ) {
    if (/\brtl\b/i.test(text) || /חזור הביתה|חזרה הביתה|תחזור הביתה/.test(text)) {
      return 'RTL';
    }
    return 'LAND';
  }
  if (
    /\brtl\b/i.test(text)
    || /חזור הביתה|חזרה הביתה|תחזור הביתה|קח אותי הביתה/.test(text)
    || /\b(come|go)\s+home\b/i.test(text)
    || /\breturn to (?:home|launch)\b/i.test(text)
  ) {
    return 'RTL';
  }
  return null;
