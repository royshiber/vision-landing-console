...
import { applyEngineerApprovedParam, applyAskFlightOp } from '../flight-actions-service.mjs';
import { getActiveConnection } from '../mavlink-connection.mjs';
import { ASK_VOICE_SAFETY_LOCK, isAskBlockedParamKey, isAskSessionFlightOpKind } from '../assist/ask-safety.mjs';
...
export function registerAssistApi(app, ctx = {}) {
  ...
  async function applyAskParamChange({ key, value, reason }) {
    const param = String(key || '').trim().toUpperCase();
    if (isAskBlockedParamKey(param)) {
      throw Object.assign(new Error('blocked early-flight param'), { status: 403, code: 'blocked_param' });
    }
    if (!ctx.db) {
      return {
        ok: true,
        applied: false,
        method: 'offline',
        note: 'אין מסד — הפרמטר לא נשלח למטוס',
      };
    }
    const mavConn = getActiveConnection?.();
    return applyEngineerApprovedParam(ctx.db, {
      sessionId: `ask:${param}`,
      key: param,
      value,
      mavConn,
  ...
  async function applyAskSessionFlightOp({ kind, mode, reason }) {
    const op = String(kind || '').toUpperCase();
    if (op === 'ARM' || op === 'DISARM' || !isAskSessionFlightOpKind(op)) {
      return {
        ok: false,
        blocked: true,
        sent: false,
        error: 'blocked_arm',
        note: ASSIST_HE.blockedFlightCommandAnswer,
      };
    }
    const mavConn = getActiveConnection?.();
    return applyAskFlightOp(ctx.db || null, {
      kind: op,
      mode,
      sessionId: `ask-op:${op}:${String(reason || '').slice(0, 24)}`,
      mavConn,
      appVersion: ctx.APP_VERSION || null,
      fcFirmware: mavConn?.autopilotName || null,
    });
  ...
  const assist = ctx.assistService || createAssistService({
    ...
    applyParamChange: ctx.applyParamChange || applyAskParamChange,
    applyFlightOp: ctx.applyFlightOp || applyAskSessionFlightOp,
  });
  ctx.assistService = assist;
  ...
  app.get('/api/assist/session', (_req, res) => {
    ...
    res.json({
      ok: true,
      session,
      ask_voice_go_active: session.ask_voice_go_active === true,
      ask_voice_safety_lock: ASK_VOICE_SAFETY_LOCK,
    });
  });

  app.post('/api/assist/voice-go', (req, res) => {
    const body = req.body && typeof req.body === 'object' ? req.body : {};
    const active = body.active === true || body.go === true;
    const next = assist.setAskVoiceGo(active);
    res.json({
      ok: true,
      ask_voice_go_active: next,
      ask_voice_safety_lock: ASK_VOICE_SAFETY_LOCK,
      answer: next ? ASSIST_HE.voiceGoEnabled : ASSIST_HE.voiceGoDisabled,
    });
  });
  ...
  app.post('/api/assist/confirm', async (req, res) => {
    try {
      ...
      const start_agent = body.start_agent !== false && body.startAgent !== false && body.mode !== 'draft';
      const result = await assist.confirmProposal({ proposal_id, confirm, start_agent });
