import { registerCoreApi } from './core-api.mjs';
import { registerFlightLogsApi } from './flight-logs-api.mjs';
import { registerAdvisorApi } from './advisor-api.mjs';
import { registerFeatureDesignerApi } from './feature-designer-api.mjs';
import { registerFlightEngineerApi } from './flight-engineer-api.mjs';
import { registerSimLabApi } from './sim-lab-api.mjs';
import { registerCompanionProxyApi } from './companion-proxy-api.mjs';
import { registerCompanionConnectionApi } from './companion-connection-api.mjs';
import { registerDevelopmentTasksApi } from './development-tasks-api.mjs';
import { registerDevelopChatApi } from './develop-chat-api.mjs';
import { registerAssistApi } from './assist-api.mjs';
import { registerArmDisarmApi } from './arm-disarm-api.mjs';
import { registerDualLinkApi } from './dual-link-api.mjs';
import { registerVisionLandingReadinessApi } from './vision-landing-readiness-api.mjs';
import { registerVersionsApi } from './versions-api.mjs';
import { registerAppUpdateApi } from './app-update-api.mjs';
import { resolveDevelopmentRuntime } from '../development-runtime.mjs';

/** Why: split core (telemetry, flights, connections, …) from advisor (chat, issues, apply, audit) from feature-designer. */
export function registerHttpRoutes(app, ctx) {
  resolveDevelopmentRuntime(ctx);
  registerCoreApi(app, ctx);
  registerFlightLogsApi(app, ctx);
  registerDualLinkApi(app, ctx);
  registerVisionLandingReadinessApi(app, ctx);
  registerVersionsApi(app, ctx);
  registerAppUpdateApi(app, ctx);
  registerCompanionProxyApi(app, ctx);
  registerCompanionConnectionApi(app, ctx);
  registerAdvisorApi(app, ctx);
  registerFeatureDesignerApi(app, ctx);
  registerFlightEngineerApi(app, ctx);
  registerSimLabApi(app, ctx);
  registerDevelopmentTasksApi(app, ctx);
  registerDevelopChatApi(app, ctx);
  registerAssistApi(app, ctx);
  registerArmDisarmApi(app);
}
