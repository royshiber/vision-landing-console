...
export function registerDevelopmentTasksApi(app, ctx = {}) {
  ...
  const testingProvider = runtime.testingProvider;
  const worktreeManager = runtime.worktreeManager;
  const agentService = runtime.agentService;
  const testingService = runtime.testingService;
  const releaseBuilder = ctx.releaseBuilder || createDevelopmentReleaseBuilder({ repoRoot: runtime.repoRoot });
  ...
  app.get('/api/development/tasks/meta', async (_req, res) => {
    const named = provider.providerName !== 'unavailable' && !String(provider.providerName).startsWith('unavailable');
    let runtime = null;
    try {
      if (typeof provider.getRuntimeStatus === 'function') runtime = await provider.getRuntimeStatus();
    } catch (err) {
      runtime = { ok: false, reason: String(err?.message || 'Development agent unavailable') };
    }
    const agentAvailable = runtime ? runtime.ok === true : named;
    res.json({
      ok: true,
      statuses: DEV_TASK_STATUSES,
      priorities: DEV_TASK_PRIORITIES,
      taxonomies: DEV_TASK_TAXONOMIES,
      targetAreas: DEV_TASK_TARGET_AREAS,
      localStore: 'var/development/tasks.json',
      agentProvider: provider.providerName,
      agentAvailable,
      agentRuntime: agentAvailable ? 'READY' : 'UNAVAILABLE',
      agentUnavailableReason: agentAvailable
        ? null
        : (runtime?.reason || provider.unavailableReason || 'Development agent unavailable'),
