...
export function registerDevelopmentTasksApi(app, ctx = {}) {
  ...
  app.get('/api/development/tasks', (req, res) => {
    try {
      ...
      res.status(500).json({ ok: false, message: err?.message || 'cannot list development tasks' });
    }
  });
  ...
  app.get('/api/development/tasks/:id', (req, res) => {
    try {
      ...
      return res.status(500).json({ ok: false, message: err?.message || 'cannot read task' });
    }
  });
