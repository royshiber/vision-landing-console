...
import { projectRoot, getConfig, setConfig } from '../db.mjs';
import {
  buildAutoConfigRecipe,
  listComponentTypes,
  listHardwareIntentComponents,
} from '../auto-config-recipes.mjs';
import { listConceptBCatalog } from '../auto-config-hardware.mjs';
import { listPeripheralWizardCatalog } from '../fc-peripheral-wizard.mjs';
import { listFcParamGroups } from '../fc-param-groups.mjs';
import { fcPresentationsForKeys } from '../fc-param-descriptions.mjs';
import {
  listFcParamFiles,
  readFcParamFile,
...
import { buildSseMavlinkSnapshot } from '../sse-mavlink-snapshot.mjs';
...
import { mergeTelemetryWithCompanion } from '../companion-events-bridge.mjs';

/** Why: SQLite key-value snapshot for GET/POST /api/vision/config. What: RAM stays the process working store; these keys survive Node restart. */
...
/** Why: restore last successful POST after process start. What: merge server_config JSON onto RAM defaults; skip missing/corrupt rows. */
function hydrateVisionConfigFromDb(ctx) {
  if (!isPlainObject(ctx.visionProfileStore)) ctx.visionProfileStore = {};
  if (!isPlainObject(ctx.arduTargetParams)) ctx.arduTargetParams = { ...buildArduTargetDefaults() };
  if (!ctx.db) return;
  try {
    const savedProfile = getConfig(ctx.db, VISION_CONFIG_PROFILE_KEY, null);
    if (isPlainObject(savedProfile)) {
      ctx.visionProfileStore = { ...ctx.visionProfileStore, ...savedProfile };
    }
    const savedArdu = getConfig(ctx.db, VISION_CONFIG_ARDU_KEY, null);
    if (isPlainObject(savedArdu)) {
      ctx.arduTargetParams = { ...ctx.arduTargetParams, ...savedArdu };
    }
  } catch (err) {
    logger.warn({ err }, 'GET/POST /api/vision/config hydrate from server_config failed');
  }
}

/** Why: durable snapshot after coerce/merge. What: upsert RAM profile + arduTarget; does not touch FC / simulated WRITE. */
function persistVisionConfigToDb(ctx) {
  if (!ctx.db) return;
  setConfig(ctx.db, VISION_CONFIG_PROFILE_KEY, ctx.visionProfileStore || {});
  setConfig(ctx.db, VISION_CONFIG_ARDU_KEY, ctx.arduTargetParams || {});
}

/** Why: param-set success already mutated RAM; snapshot must not fail the HTTP success payload. */
function persistVisionConfigToDbAfterParamSet(ctx) {
  try {
    persistVisionConfigToDb(ctx);
  } catch (err) {
    logger.error({ err }, 'POST /api/param-center/param-set persist to server_config failed');
  }
}
...
app.get('/api/health', (_req, res) => {
  ...
  res.json({
    ...
    companion: ctx.companionService?.describe?.() || {
      lane: 'NEW',
      mode: 'off',
      api: 'v1',
 ...
 *      via MAVLink when connected, or into the RAM target when offline.
 * What: POST { param, value } → writes one param, snapshots server_config when RAM target changes.
 */
app.post('/api/param-center/param-set', async (req, res) => {
  ...
  if (mavConn?.connected && typeof mavConn.setParam === 'function') {
    try {
      ...
      if (!result?.ok) {
        return res.status(500).json({ ok: false, message: result?.error || 'ה-FC לא אישר את הכתיבה' });
      }
      ...
      if (ctx.arduTargetParams && param in ctx.arduTargetParams) {
        ...
        persistVisionConfigToDbAfterParamSet(ctx);
      }
  ...
  // Offline path — RAM + SQLite snapshot (no FC verification)
  if (!mavConn?.connected) {
    if (ctx.arduTargetParams) {
      ctx.arduTargetParams[param] = value;
      persistVisionConfigToDbAfterParamSet(ctx);
    }
    ...
    return res.json({
      ...
      value,
      param,
      warning: 'MAVLink לא מחובר. הערך נשמר זמנית בשרת בלבד. חברו ל-FC ובצעו כתיבה כדי לשלוח לבקר.',
    });
  }
...
function pushSseTelemetrySnapshot() {
  ...
  const mavConn = getActiveConnection?.();
  const mavlink = buildSseMavlinkSnapshot(mavConn);
  ...
  broadcastSse(
    'telemetry',
    mergeTelemetryWithCompanion(legacyTelemetry, ctx.companionService?.getSseOverlay?.() || null),
  );
}
...
app.get('/api/stream', (req, res) => {
  ...
  res.flushHeaders();
  sseClients.add(res);
  try { pushSseTelemetrySnapshot(); } catch (err) {
    logger.warn({ err: err?.message }, 'SSE immediate snapshot failed');
  }
  req.on('close', () => sseClients.delete(res));
});

setInterval(pushSseTelemetrySnapshot, 300);
...
/** Why: unified WRITE — persist console profile and editable ArduPilot targets before hardware WRITE. What: shallow-merge body fields into server stores, then snapshot SQLite. */
app.post('/api/vision/config', (req, res) => {
  ...
  try {
    ...
    logger.error({ err }, 'POST /api/vision/config persist to server_config failed');
  }
...
function rememberFcParamFile(entry) {
  try {
    saveFcParamFile(resolveFcParamFilesDir(ctx.db), entry);
  } catch (err) {
    logger.warn({ err: err?.message }, 'fc param file save failed');
  }
}

/** Why: the parameters dropdown lists real ArduPlane names. Values are not included. */
app.get('/api/ardu/param-groups', (_req, res) => {
  const groups = listFcParamGroups();
  const keys = groups.flatMap((group) => group.keys);
  res.json({ ok: true, groups, meta: fcPresentationsForKeys(keys) });
});

/** Why: history is metadata only. The full snapshot is fetched for an explicit restore. */
app.get('/api/ardu/param-files', (_req, res) => {
  res.json({ ok: true, files: listFcParamFiles(resolveFcParamFilesDir(ctx.db)) });
});

app.get('/api/ardu/param-files/:id', (req, res) => {
  const file = readFcParamFile(resolveFcParamFilesDir(ctx.db), req.params.id);
  if (!file) return res.status(404).json({ ok: false, message: 'הקובץ לא נמצא' });
  res.json({
    ok: true,
    file: {
      id: file.id,
...
/** Why: READ needs current device state vs editable target. Live MAVLink never falls back to a simulated RAM copy. */
app.get('/api/ardu/params', (req, res) => {
  const { armed, mavlinkConnected } = getArmedGateStatus();
  const mavConn = getActiveConnection?.();
  ...
  if (shouldRecordFcRead({
    record: String(req.query.record || '') === '1',
    mavlinkConnected,
    connected,
    current,
  })) {
    rememberFcParamFile({ source: 'read', params: { ...current }, armed });
  }
...
/** Why: WRITE sends only operator-dirty keys via MAVLink setParam; empty body is not a template blast. */
app.post('/api/ardu/params/write', async (req, res) => {
  const mavConn = getActiveConnection?.();
  const mavlinkConnected = !!mavConn?.connected;
  if (mavlinkConnected) {
  ...
  const requestedRaw = resolveRequestedWriteParams(req.body);
  const normalized = coerceArduTargetPatch(requestedRaw, ctx.arduTargetParams);
  ...
  if (mavlinkConnected && typeof mavConn.setParam === 'function') {
    const live = getConnectionParams(mavConn.id) || mavConn.params || {};
    const items = selectChangedEditableParams(normalized.accepted, live);
    if (items.length === 0) {
      return res.json({
        ...emptyWrite,
        via: 'mavlink',
    ...
    const bulkReject = rejectIfOverBulkCap(items, req.body?.confirmBulk);
    if (bulkReject) {
    ...
    try {
      const { written, failed, verified } = await writeEditableParamsViaMavlink(mavConn, items);
      if (Object.keys(verified).length) {
        ctx.arduCurrentParams = { ...(ctx.arduCurrentParams || {}), ...verified };
      }
      const ok = failed.length === 0;
      if (written.length > 0) {
        rememberFcParamFile({
          source: 'write',
          params: { ...(live && typeof live === 'object' ? live : {}), ...verified },
          armed: getArmedGateStatus().armed,
        });
      }
      return res.status(ok ? 200 : 207).json({
        ...
        skipped: Object.keys(normalized.accepted).length - items.length,
        failed,
        verified,
        rejected: normalized.rejected,
        message: ok
          ? `WRITE SUCCESS — ${written.length} פרמטרים נכתבו ל-FC`
          : `WRITE חלקי — ${written.length} נכתבו, ${failed.length} נכשלו`,
      });
    } catch (err) {
      logger.warn({ err: err?.message }, 'POST /api/ardu/params/write MAVLink failed');
      return res.status(500).json({
        ok: false,
        via: 'mavlink',
        simulated: false,
        message: err?.message || 'שגיאת MAVLink',
      });
    }
  }

  const offlineItems = selectChangedEditableParams(normalized.accepted, ctx.arduCurrentParams);
  ...
  const offlineBulk = rejectIfOverBulkCap(offlineItems, req.body?.confirmBulk);
  if (offlineBulk) {
    return res.status(400).json({
      ...offlineBulk,
      via: 'offline',
  ...
  ctx.arduCurrentParams = { ...(ctx.arduCurrentParams || {}), ...normalized.accepted };
  const n = offlineItems.length;
  if (n > 0) {
    rememberFcParamFile({
      source: 'write',
      params: { ...ctx.arduCurrentParams },
      armed: getArmedGateStatus().armed,
    });
  }
  return res.json({
...
app.post('/api/connections/quick-connect', async (req, res) => {
  try {
    ...
    db.prepare(`UPDATE connections SET active = 0 WHERE COALESCE(link_role, 'radio') = 'radio'`).run();
    ...
    ).run(name, type, rowHost, rowPort, rowSerial, br, 'radio');
...
/** Return supported component types plus the Concept B wizard catalog. */
app.get('/api/auto-config/components', (_req, res) => {
  res.json({
    ok: true,
    components: listComponentTypes(),
    hardwareIntent: listHardwareIntentComponents(),
    conceptB: listConceptBCatalog(),
    peripheralWizard: listPeripheralWizardCatalog(),
  });
});
