...
import { applySimulatorPreset, simulatorConnectFailureMessage } from '../sim-vehicle.mjs';
...
app.post('/api/connections/quick-connect', async (req, res) => {
  try {
    const body = req.body || {};
    const shaped = applySimulatorPreset({
      type: body.type,
      host: body.host,
      port: body.port,
    ...
    const serialPort = shaped.serialPort;
    const host = shaped.host;
    const port = shaped.port;
    ...
    if (shaped.simulatorPreset === true) {
      name = shaped.name;
      rowHost = shaped.host;
      rowPort = shaped.port;
    } else if (type === 'serial') {
    ...
    try {
      await activateConnection({
        ...
        baudRate: br,
        linkRole: 'radio',
        simulatorPreset: shaped.simulatorPreset === true,
      });
    } catch (err) {
      ...
      const simMessage = simulatorConnectFailureMessage(err, shaped.simulatorPreset === true);
      return res.status(500).json({ ok: false, message: simMessage || err?.message || 'הפעלת חיבור נכשלה' });
    }
