...
/** Why: WRITE sends editable targets via MAVLink setParam when connected; RAM copy only when disconnected. */
app.post('/api/ardu/params/write', async (_req, res) => {
  const mavConn = getActiveConnection?.();
  const mavlinkConnected = !!mavConn?.connected;
  if (mavlinkConnected) {
  ...
  if (mavlinkConnected && typeof mavConn.setParam === 'function') {
    ...
    if (items.length === 0) {
      return res.json({
        ...
        simulated: false,
        written: 0,
        skipped: Object.keys(normalized.accepted).length,
        failed: [],
        verified: {},
        rejected: normalized.rejected,
        message: 'WRITE — אין שינוי מול הבקר',
      });
    }
    try {
  ...
  const n = Object.keys(normalized.accepted).length;
  return res.json({
    ok: true,
    via: 'offline',
    simulated: true,
    ...
    failed: [],
    rejected: normalized.rejected,
    message: `WRITE SUCCESS — ${n} פרמטרים נכתבו (סימולציה — אין MAVLink)`,
  });
});
