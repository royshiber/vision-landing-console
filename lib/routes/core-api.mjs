...
import { mergeTelemetryWithCompanion } from '../companion-status.mjs';
...
app.get('/api/health', (_req, res) => {
  ...
  res.json({
    ...
    companion: typeof companionService?.describe === 'function'
      ? companionService.describe()
      : { mode: 'off' },
  });
});
...
/** LEGACY: heartbeat / status / reboot on /api/jetson and /api/rpi — freeze; new reads use /api/jetson/v1. */
...
setInterval(() => {
  ...
  const overlay = companionService?.getBridge?.()?.getLastOverlay?.() || null;
  broadcastSse('telemetry', mergeTelemetryWithCompanion({
