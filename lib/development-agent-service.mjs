...
function extractPublicHttpUrl(snapshot) {
  if (!snapshot || typeof snapshot !== 'object') return null;
  const keys = ['pr_url', 'prUrl', 'pull_request_url', 'pullRequestUrl', 'url', 'agent_url', 'target_url'];
...
export function createDevelopmentAgentService({ store, provider, worktreeManager }) {
  ...
  async function getTaskAgent(taskId) {
    ...
    return {
      ...synced.agent,
      pr_url: extractPublicHttpUrl(snapshot),
    };
  }
