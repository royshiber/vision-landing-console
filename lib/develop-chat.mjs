...
function installFor(session) {
  ...
  const progress = session.install?.progress
    ...
    : emptyProgress();
  if (ready) {
    if (progress.jetson_upload.status === 'idle' || progress.jetson_upload.status === 'queued') {
      progress.jetson_upload = { status: 'allowed', percent: 100 };
    }
    if (progress.fc_install.status === 'idle' || progress.fc_install.status === 'queued') {
      progress.fc_install = { status: 'allowed', percent: 100 };
    }
  }
  return {
    authorized: ready,
...
function authorizeInstall(session) {
  enableGates(session);
  session.gates.jetson_upload.approved = true;
  session.gates.fc_install.approved = true;
  session.gates.jetson_upload.applied = false;
  session.gates.fc_install.applied = false;
  session.install = {
    authorized: true,
    note: 'אין פקודת טיסה.',
    jetson: { target: 'מחשב משימה', items: [] },
    fc: { target: 'בקר טיסה', items: [] },
    progress: {
      jetson_upload: { status: 'allowed', percent: 100 },
      fc_install: { status: 'allowed', percent: 100 },
    },
  };
}
...
export function markDevelopChatReady(session, extras = {}) {
  ...
  authorizeInstall(session);
  pushMessage(
    session,
    'assistant',
    extras.landed
      ? 'היכולת נחתה. למטה מוצג מה יותקן. ההתקנה מותרת. אין החלה על חומרה חיה ואין פקודת טיסה.'
      : 'הקוד מוכן לתצוגה. למטה מוצג מה יותקן. ההתקנה מותרת. אין החלה על חומרה חיה ואין פקודת טיסה.',
    { gates: true },
  );
  session.updated_at = nowIso();
  return snapshotDevelopChat(session);
