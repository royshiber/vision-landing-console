...
function previewFor(session) {
  ...
  return {
    ...
    after: {
      kicker: 'אחרי',
      headline: title,
      lines: [`משטח: ${surfaceLabel}`, afterHint, 'תצוגה לקריאה בלבד עד אישור שער.'],
    },
  };
}
...
export function applyDevelopChatTurn(session, input = {}) {
  ...
  if (session.phase === 'idle') {
    ...
    pushMessage(session, 'assistant', 'קיבלתי. אשאל שתי שאלות קצרות ואז אבנה. אפשר תמיד לענות בטקסט חופשי.');
  ...
  if (session.phase === 'awaiting_build') {
    ...
    pushMessage(session, 'assistant', 'היכולת מוכנה לתצוגה. השערים דורשים אישור אנושי — אין החלה אוטומטית.');
  } else {
...
export function markDevelopChatReady(session, extras = {}) {
  ...
  pushMessage(
    ...
    'assistant',
    extras.landed
      ? 'היכולת נחתה. הפרמטרים זמינים בכרטיס פרמטרים. השערים דורשים אישור אנושי.'
      : 'הקוד מוכן לתצוגה. אשר העלאה או התקנה רק אם אתה רוצה — אין החלה אוטומטית.',
    { gates: true },
  );
  session.updated_at = nowIso();
  return snapshotDevelopChat(session);
...
export function approveDevelopChatGate(session, gateId) {
  ...
  pushMessage(
    ...
    gateId === DEVELOP_GATE_JETSON.id
      ? 'נרשם אישור העלאה למחשב משימה. אין העלאה בפועל עד שער נפרד מחוץ למסך הזה.'
      : 'נרשם אישור התקנה לבקר. אין כתיבה לבקר מהמסך הזה.',
