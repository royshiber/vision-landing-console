...
import { CompanionApiError, parseCompanionBaseUrls } from './companion-api-client.mjs';
...
export const COMPANION_HE = Object.freeze({
  ...
  tokenEmpty: 'חסר טוקן',
  tokenMissingHint: 'חסר טוקן. הזינו אותו במתקדם.',
  ...
  unauthorized: 'הטוקן נדחה. בדקו את הטוקן ונסו שוב.',
  bothGate: 'כתובת לבד לא מספיקה. צריך גם טוקן.',
  httpError: 'שגיאת שרת במחשב משימה',
  parse: 'תשובת מחשב משימה אינה תקינה',
  hint: 'צריך כתובת וטוקן. כתובת לבד לא מספיקה.',
});
...
export function companionLinkLabelHe(url) {
...
export function validateCompanionBaseUrl(raw) {
  const parts = parseCompanionBaseUrls(raw);
  if (!parts.length) {
...
export function snapshotCompanionEnv(env = process.env) {
  ...
  return {
    ...
    JETSON_COMPANION_BASE_URLS: normalizeCompanionSecret(env.JETSON_COMPANION_BASE_URLS),
    JETSON_COMPANION_SOCKS_PROXY: String(env.JETSON_COMPANION_SOCKS_PROXY || '').trim(),
    JETSON_SOCKS_PROXY: String(env.JETSON_SOCKS_PROXY || '').trim(),
    JETSON_COMPANION_TOKEN: token,
    COMPANION_SHARED_SECRET: token,
...
export function buildPublicCompanionConnectionStatus({
  ...
  const activeUrl = (desc.activeBaseUrl && desc.activeBaseUrl !== 'mock://companion')
    ? desc.activeBaseUrl
    : (configuredReal && desc.baseUrl && desc.baseUrl !== 'mock://companion' ? desc.baseUrl : null);
  const payload = {
    ok: true,
    mode,
    connected,
    ...
    active_base_url: activeUrl,
    active_link_he: companionLinkLabelHe(activeUrl),
    base_url_candidates: Array.isArray(desc.baseUrlCandidates) ? desc.baseUrlCandidates : [],
...
export function snapshotCompanionLinkFromCtx(ctx = {}) {
  ...
  const active = desc.activeBaseUrl && desc.activeBaseUrl !== 'mock://companion' ? desc.activeBaseUrl : null;
  return applyMavlinkRelayHint({
    ...
    active_link_he: companionLinkLabelHe(active),
    base_url_candidates: Array.isArray(desc.baseUrlCandidates) ? desc.baseUrlCandidates : [],
