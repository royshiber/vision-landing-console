...
export function isVehicleFlying(conn, now = Date.now()) {
  return flightStateForDisarm(conn, now).flying === true;
}

/**
 * Fail closed. A missing or stale EXTENDED_SYS_STATE is not "on the ground".
 * Disarm then needs the same second confirm as a known airborne vehicle.
 */
export function flightStateForDisarm(conn, now = Date.now()) {
  const raw = conn?.lastLandedState;
  const state = Number(raw);
  const at = Number(conn?.lastLandedStateAt);
  const fresh = Number.isInteger(raw) && Number.isFinite(at) && (now - at) <= LANDED_STATE_FRESH_MS;
  if (!fresh) {
    return { known: false, flying: null, uncertain: true, needsSecondConfirm: true };
  }
  const flying = state === LANDED_STATE_IN_AIR || state === LANDED_STATE_TAKEOFF;
  return { known: true, flying, uncertain: false, needsSecondConfirm: flying };
}
