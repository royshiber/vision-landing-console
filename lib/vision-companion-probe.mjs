...
function visionNeedsCompanionFetch(vision, extras) {
  if (!obj(vision) && !obj(extras)) return true;
  return firstBool(
    obj(vision)?.camera_ok,
    obj(vision)?.cameraOk,
    obj(extras)?.camera_ok,
    obj(extras)?.camera_connected,
    obj(extras)?.cameraConnected,
  ) == null;
}

function videoNeedsCompanionFetch(video) {
  const v = obj(video);
  if (!v) return true;
  return v.raw_pipeline == null && v.rawPipeline == null;
}

function landingNeedsCompanionFetch(landing, extras) {
  if (!obj(landing) && !obj(extras)) return true;
  if (detectorDeclared({ landing, extras }) != null) return false;
  if (explicitRunwayFlag({ landing, extras }) != null) return false;
  if (runwaySource({ landing, extras })) return false;
  if (obj(obj(landing)?.runway) || obj(obj(extras)?.runway)) return false;
  return true;
}

/**
 * Overlay first; fill missing vision/landing/video from Companion GET paths.
 * Empty mapped skeletons (no camera_ok / runway fields) still fetch.
...
export async function collectCompanionVisionSignals({ overlay, client } = {}) {
  ...
  let opticalNav = obj(o.optical_nav) || obj(o.opticalNav) || obj(extras)?.optical_nav || obj(extras)?.opticalNav || null;
  let landingPathAbsent = false;
  const live = { vision: false, video: false, landing: false, visionResult: false, opticalNav: false, cameras: false };

  if (client) {
    const needVision = visionNeedsCompanionFetch(vision, extras);
    const needVideo = videoNeedsCompanionFetch(video);
    const needLanding = landingNeedsCompanionFetch(landing, extras);
    const needOptical = !obj(opticalNav)?.cameras;
    const [vRead, vidRead, lRead, rRead, nRead] = await Promise.all([
      ...
      needOptical && typeof client.getStatusOpticalNav === 'function'
        ? readCompanionPath(client.getStatusOpticalNav.bind(client))
    ...
    if (nRead?.ok && obj(nRead.data)) {
      opticalNav = { ...opticalNav, ...obj(nRead.data) };
      live.opticalNav = true;
      extras = { ...(extras || {}), optical_nav: opticalNav };
    }
    if (typeof client.getStatusCameras === 'function') {
      const camRead = await readCompanionPath(client.getStatusCameras.bind(client));
      const fetched = obj(camRead?.data)?.cameras;
      if (camRead?.ok && fetched) {
        const prev = obj(extras?.cameras) || {};
        const merged = { ...fetched };
        for (const [key, value] of Object.entries(prev)) {
          if (value && typeof value === 'object') merged[key] = { ...(merged[key] || {}), ...value };
        }
        extras = { ...(extras || {}), cameras: merged };
        live.cameras = true;
      }
    }
  }
