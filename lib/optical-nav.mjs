 ...
 * Roy LOCKED 2026-09-12: nav_toggle_display_only.
 * GPS | אופטי is a console display/logging preference.
 * Never commands FC/EKF source switch. Never invents WGS84. Never enables EKF inject.
 */
...
/** Roy LOCKED: Mission toggle must stay display/logging. Live nav-source switch is a later Human Gate. */
export const NAV_TOGGLE_MODE = 'display_only';
export const NAV_TOGGLE_LOCK = 'nav_toggle_display_only';
...
function cameraStub(raw, role) {
  const cam = obj(raw) || {};
  const sourceRaw = String(cam.source || '').trim().toLowerCase();
  const source = sourceRaw === 'real' || sourceRaw === 'synthetic' || sourceRaw === 'absent'
    ? sourceRaw
    : (cam.dry_run === true || cam.dryRun === true ? 'absent' : (boolOrNull(cam.camera_ok ?? cam.cameraOk) === true ? 'real' : 'absent'));
  const dryRun = cam.dry_run === true || cam.dryRun === true || source === 'synthetic';
  const fps = finiteOrNull(cam.fps);
  const ageMs = finiteOrNull(cam.last_frame_age_ms ?? cam.lastFrameAgeMs ?? cam.frame_age_ms ?? cam.age_ms);
  const frames = finiteOrNull(cam.frame_count ?? cam.frameCount ?? cam.frames);
  const present = boolOrNull(cam.present);
  return {
    ...
    error: typeof cam.error === 'string' && cam.error.trim() ? cam.error.trim() : null,
    source,
    dry_run: dryRun,
    real: source === 'real' && dryRun === false && boolOrNull(cam.camera_ok ?? cam.cameraOk) === true,
  };
}
...
export function emptyOpticalNav(overrides = {}) {
  return {
    ...
    confidence: null,
    ekf_injected: false,
    display_only: NAV_TOGGLE_MODE === 'display_only',
    cameras: {
      cam1: {
        id: 'cam1',
        role: 'vio_forward',
