...
export function createCompanionApiClient(opts = {}) {
  ...
  function hostInterfaces() {
    if (typeof opts.networkInterfaces === 'function') {
      try { return opts.networkInterfaces() || {}; } catch { return null; }
    }
    if (opts.networkInterfaces && typeof opts.networkInterfaces === 'object') {
      return opts.networkInterfaces;
    }
    try { return os.networkInterfaces(); } catch { return null; }
  }
  ...
  async function probeOne(candidate) {
    ...
    try {
      ...
      return {
        ...
        status: null,
        rttMs: null,
        errorHe: lanProbeErrorHe(candidate, err, hostInterfaces()),
      };
    } finally {
      clearTimeout(timer);
    }
