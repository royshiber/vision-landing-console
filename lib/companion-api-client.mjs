...
export function resolveCompanionV1BaseUrl(env = process.env) {
  const raw = normalizeCompanionSecret(env.JETSON_COMPANION_BASE_URL).replace(/\/+$/, '');
  return raw || null;
}
...
export function createCompanionApiClient(opts = {}) {
  ...
  const client = {
    ...
    /**
     * Latest JPEG for cam1/cam2. 404 / empty is honest אין פריים — never invent a frame.
     * @param {'cam1'|'cam2'|string} camId
     */
    getCameraFrame: async (camId) => {
      const key = String(camId || '').trim().toLowerCase() === 'cam2' ? 'cam2' : 'cam1';
      const pathname = key === 'cam2' ? COMPANION_V1_PATHS.cameraFrameCam2 : COMPANION_V1_PATHS.cameraFrameCam1;
      return requestBytes(pathname);
    },
    ...
    /** Read-only uplink report. Not part of the mock method list. */
    getNetworkUplinks: () => request(COMPANION_V1_PATHS.networkUplinks),
    /**
     * POST /api/v1/network/uplinks/{wifi|cellular} { enabled }.
     * Console calls this only after health.capabilities.uplinkControl.
     * @param {'wifi'|'cellular'|string} which
     * @param {boolean} enabled
     */
    postNetworkUplink: (which, enabled) => {
      const role = which === 'cellular' ? 'cellular' : 'wifi';
      return request(`${COMPANION_V1_PATHS.networkUplinks}/${role}`, {
        method: 'POST',
