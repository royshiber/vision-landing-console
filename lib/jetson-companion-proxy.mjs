/**
 * LEGACY Jetson companion HTTP (heartbeat-era agent on :8081).
 * Do not add aircraft logic here. Companion API v1 lives in lib/companion-api-client.mjs.
 */
import { readFile, writeFile, mkdir } from 'fs/promises';
import path from 'path';
import { projectRoot } from './db.mjs';
import { readCompanionTokenFromEnv } from './companion-secret.mjs';
import { jetsonFetch } from './jetson-socks.mjs';

export const DEFAULT_HTTP_PORT = 8081;
export const DEFAULT_RELAY_PORT = 5770;

/** Host of the one Jetson MAVLink TCP relay, derived from Companion HTTP base URL. */
export function relayHostFromCompanionBaseUrl(baseUrl) {
  const raw = String(baseUrl || '').split(',')[0].trim();
  if (!raw) return null;
  try {
    const u = new URL(raw);
...
/**
 * TCP target for companion_agent.py VLC_RELAY_PORT (default 5770).
 * Companion HTTP connect does not open this session by itself.
 */
export function companionMavlinkRelayTarget(baseUrl, relayPort = DEFAULT_RELAY_PORT) {
  const host = relayHostFromCompanionBaseUrl(baseUrl);
  const port = Number(relayPort);
  const resolvedPort = Number.isFinite(port) && port > 0 ? port : DEFAULT_RELAY_PORT;
  if (!host) return null;
  return {
    type: 'tcp',
    host,
    port: resolvedPort,
    label: `Jetson MAVLink relay (${host}:${resolvedPort})`,
  };
}
...
export function getCompanionBaseUrl(jetsonState, envBaseUrl = '') {
  const env = String(envBaseUrl || '').split(',')[0].trim().replace(/\/+$/, '');
  const envIsCompanionV1 = /\/api\/v1$/i.test(env);
  if (env && !envIsCompanionV1) return env;
...
export function companionAuthHeaders() {
  const token = readCompanionTokenFromEnv(process.env);
...
export async function downloadCompanionLog(baseUrl, name) {
  ...
  const r = await jetsonFetch(url, { headers: companionAuthHeaders(), signal: AbortSignal.timeout(120_000) });
...
export function buildJetsonRelayTargets(jetsonState, env = {}, companionBaseUrl = '') {
  ...
  const fromCompanion = companionMavlinkRelayTarget(
    companionBaseUrl || env.JETSON_COMPANION_BASE_URL || jetsonState?.companionHttpUrl,
    relayPort,
  );
  if (fromCompanion) add(fromCompanion.host, fromCompanion.port, fromCompanion.label);
