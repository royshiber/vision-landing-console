...
describe('Telemetry dash MERGE into Pulse + Platform diagnostics', () => {
  it('adds a Platform אבחונים shell that opens existing #telemetry', () => {
    ...
    const src = [
      'const tabs = { telemetry: false, platform: false };',
    ...
    expect(tabs.platform).toBe(true);
    expect(sliceFunction(js, 'initPlatformShell')).toMatch(/operatorOpenFirstAction\(go\.dataset\.platformGo\)/);
    expect(sliceFunction(js, 'platformRefresh')).toMatch(/platformDiagStatus/);
    expect(sliceFunction(js, 'platformDiagLabel')).toMatch(/teleStatusBadge/);
  });
