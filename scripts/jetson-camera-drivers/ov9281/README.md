...
tegracam clamps an INTEGER64 control that starts at 0 up to the minimum, so a fresh node used to report `exposure=10` and `frame_rate=10000000` even when the devicetree default was higher. Probe writes 10000 / 64 / 60000000 into those controls. A write while the sensor is off only updates that cache. Stream start writes the cache again after the software reset. Each exposure, gain, and frame-rate update is launched through `0x3208` (`0x00`, then `0x10`, then `0xa0`). `0x3308` does not commit the new exposure on this sensor, so the picture stayed on the previous integration time. The `ov9281 timing` line includes `3500`..`3502`, `3509`, `380e`/`380f`, `3503`, and `3208`. A mismatch between the value just written and those bytes is `ov9281 exposure readback` / `ov9281 gain readback` in dmesg. An idle read of `ov9281_timing` returns that last line with `source=cached` when the sensor is powered down (`-121`).
...
**Exposure and gain read back from v4l2-ctl but the picture does not change.** The control cache is not the sensor. After a capture, `ov9281 timing stream` must show `3500`..`3502` and `3509` matching the `ov9281 exposure` / `ov9281 gain` lines, and there must be no `exposure readback` error. `3208` in that line is the group-update register after the launch (`0xa0`). A flat dark frame with the lens cap on is not this bug. On this board the 10-bit sample is `value >> 4` inside the 16-bit word; `frame_stats.py` prints that as `alignment shift`.
