...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  echo "{\"ok\":true,\"applied\":false,\"flightlog_unit\":false,\"backup\":\"${DEST}.backups/YYYYMMDDTHHMMSSZ\",\"manifest\":\"version,deployed_at,git_sha,known_good\",\"note\":\"dry-run; nothing copied. A real apply copies the current tree to a timestamped backup beside it and writes airvix-deploy.json. uplink-nm.sh sudo target is the root-owned /opt path, not the home copy. --enable-flightlog does not install or start the unit in dry-run\"}"
  exit 0
fi
