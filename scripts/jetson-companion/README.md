...
6. Start the 2.6.5 agent (UART fan-out unchanged):
...
6. Restart the agent as `royshiber`. `GET /api/health` reports `agentVersion` `2.6.5`.

## Version backups

`install.sh --apply` copies the current `~/vlc-companion` tree to `~/vlc-companion.backups/YYYYMMDDTHHMMSSZ` before it replaces files. Each backup and the live tree carry `airvix-deploy.json` with `version`, `deployed_at`, `git_sha`, and `known_good`. Dry-run only prints that plan. The agent lists those backups on `GET /api/v1/versions` and restores one on `POST /api/v1/versions/rollback` after an explicit confirm. Restore swaps the directory, restarts `airvix-companion`, and swaps back if health does not return in about 30 seconds. It refuses while the flight controller reports armed.
