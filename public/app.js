...
const PARAMS = [
  ...
  { key: 'vision_enable_alt_m', label: 'גובה הפעלת ראייה (m)', min: 5, max: 120, step: 1, value: 55 },
  { key: 'vision_conf_min', label: 'סף מינימום לתיקון ראייה', min: 0.4, max: 0.99, step: 0.01, value: 0.78 },
  { key: 'abort_conf_min', label: 'סף ביטחון מינימלי לביטול אוטומטי', min: 0.3, max: 0.95, step: 0.01, value: 0.70 },
  { key: 'abort_conf_hold_s', label: 'משך זמן מתחת לסף לפני ביטול (s)', min: 0.5, max: 8, step: 0.1, value: 2.0 },
  { key: 'abort_recover_conf', label: 'סף יציאה מביטול (חזרה)', min: 0.35, max: 0.99, step: 0.01, value: 0.76 },
  { key: 'xtrack_gain', label: 'הגבר סטייה רוחבית', min: 0.1, max: 3.5, step: 0.05, value: 1.25 },
  { key: 'yaw_align_gain', label: 'הגבר יישור כיוון', min: 0.1, max: 2.5, step: 0.05, value: 0.95 },
  ...
  { key: 'abort_max_xtrack_m', label: 'ביטול אם סטייה רוחבית גבוהה (m)', min: 0.5, max: 12, step: 0.1, value: 4.0 },
  { key: 'abort_max_heading_deg', label: 'ביטול אם שגיאת כיוון גבוהה (deg)', min: 5, max: 80, step: 1, value: 22 },
  { key: 'to_rotate_speed_ms', label: 'מהירות הרמה בהמראה (m/s)', min: 6, max: 30, step: 0.5, value: 13.0 },
  { key: 'to_pitch_deg', label: 'זווית עלייה בהמראה (deg)', min: 4, max: 20, step: 0.5, value: 11.0 },
  { key: 'to_max_crosswind_ms', label: 'רוח צד מקסימלית להמראה (m/s)', min: 1, max: 20, step: 0.5, value: 8.0 },
...
function computeChecklist(currentConfidence) {
  ...
  const checks = [
    ...
    {
      label: `ביטחון ראייה מעל סף ביטול (${Math.round(currentConfidence * 100)}% >= ${Math.round(Number(profileState.abort_conf_min || 0.7) * 100)}%)`,
...
(() => {
  ...
  async function onCompanionLinkClick() {
    ...
    const disconnect = companionLinkBtn.dataset.action === 'disconnect';
    setRowMessage('home', '');
    setRowPending('home', true);
    try {
      if (disconnect) {
        const r = await fetch('/api/companion/connection/disconnect', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (companionNeedsToken(j) || j.needAdvanced || j.error === 'need_advanced') {
          ...
          const msg = j.hint_he || j.status_he || (companionNeedsToken(j) ? 'חסר אסימון. הזינו אותו במתקדם.' : 'פתחו מתקדם רק אם חסרה כתובת.');
          if (companionLinkHint) companionLinkHint.textContent = msg;
      ...
      setDot('err');
      setRowMessage('home', err.message || String(err));
    } finally {
      setRowPending('home', false);
  ...
  function paintCommRows(links) {
    ...
    document.querySelectorAll('#commLinkRows .comm-link-row').forEach((el) => {
      ...
      if (enable && !enable.dataset.pending) enable.checked = row.enabled !== false;
      const btn = el.querySelector('.comm-link-action');
      if (btn && !btn.classList.contains('is-pending')) paintRowAction(btn, row.actionHe, row.statusHe);
      paintQuality(
  ...
  async function refreshConnectionStatus() {
    ...
    try {
      ...
      if (active) {
        ...
        paintRowAction(connBtn, 'התנתק', `מחובר ל-${active.liveStatus.remoteAddr || active.name}. לחצו להתנתקות.`);
  ...
  async function onCellularConnectClick() {
    ...
    const turnOff = cellularConnectBtn.dataset.action === 'disconnect';
    try {
      await postLinkPref('cellular', !turnOff);
    } catch {
...
function syncMissionLayoutChrome() {
  ...
  if (btn) btn.disabled = false;
  ['missionColSplit', 'missionColSplitB'].forEach((id) => {
    const el = document.getElementById(id);
